...
| `zshenv` | `~/.zshenv` | zsh (all shells, incl. non-interactive AI tool shells — PATH, cargo) |
...
`dot_claude/AGENTS.md` consistently uses `.ai/...` paths. That's the symlink present in every *work* repo (`nx`, `ocean`, `console`, etc.). **Inside this repo (`dot-ai-config`) there is no `.ai` symlink at the root** — the same content lives at `dot_ai/`.
