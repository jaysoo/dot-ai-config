...
1. Get the latest `master` and `website-23` branches: 
  - `git fetch origin master`
  - `git fetch origin website-23`
  - `git checkout master`
  - `git reset --hard origin/master`
  - `git checkout website-23`
  - `git reset --hard origin/website-23`
2. On `website-23`, get the last git commit message and store it into `/tmp/last-website-22-commit.txt`
3. Back on `master` find the git commit matching `/tmp/last-website-23-commit.txt` and remember the sha in `/tmp/last-master-sha.txt`
...
5. On `website-23` branch, starting from the earlist (bottom) commit to the latest (top) in `/tmp/commits-to-cherry-pick.txt`, run `git cherry-pick <sha>`
