...
- Source and destination may use different directory conventions (e.g. `libs/` vs `packages/`, `apps/` vs `projects/`).
- **Always prefer the destination's existing conventions.** If dest uses `packages/` for libraries and the source uses `libs/`, import source libraries into `packages/`, not `libs/`.
- If the dest has no established convention (e.g. empty workspace), ask the user which convention to adopt.
- When importing individual projects from a source `libs/foo` into a dest `packages/`, use: `nx import <source> packages/foo --source=libs/foo`
- This keeps the monorepo internally consistent and avoids confusing mixed conventions.
...
- `references/VITE.md` — Covers Vite projects (React, Vue, or both). Generic Vite guidance + framework-specific sections for deps, tsconfig, ESLint, and mixed-framework coexistence. Covers both Nx and non-Nx sources.
