...
JSON files instead of hitting the GitHub API. The orchestrator runs
`gh_*` helpers once and stashes the output here so subagents share a single
1Password auth session.
...
All files are JSON in the same shape the `gh_*` helpers emit. Use
`cat file || gh_helper_fn ...` fallback. For issue state verification,
use `jq '.[] | select(.number == N)' "$SCAN_DATA_DIR/issues/nrwl-nx-all.json"`
instead of a separate per-issue call.
...
if [ -z "$NX_OPEN" ]; then
  NX_OPEN=$(gh_issue_list_with_meta nrwl/nx open 200)
fi

CLOUD_OPEN=$(cat "$SCAN_DATA_DIR/issues/nrwl-nx-cloud-open.json" 2>/dev/null)
if [ -z "$CLOUD_OPEN" ]; then
  CLOUD_OPEN=$(gh_issue_list_with_meta nrwl/nx-cloud open 100 2>/dev/null || echo "[]")
fi
...
```bash
# Read from cache or fetch live (raw REST shape — note .reactions["+1"] and .created_at)
...
if [ -z "$ALL_ISSUES" ]; then
  ALL_ISSUES=$(gh_issue_list_state nrwl/nx all 200)
fi
...
# Use the cached all-issues file instead of per-issue API calls:
jq '.[] | select(.number == <NUMBER>) | {state, closedAt}' \
  "$SCAN_DATA_DIR/issues/nrwl-nx-all.json" 2>/dev/null \
  || gh_issue_view nrwl/nx <NUMBER>
```
