...
- **Sources 1 and 3** (GitHub repos): Use the `gh_*` Bash helpers (fast).
...
```bash
# List releases for the target month (helper emits tagName/publishedAt/body)
gh_release_list nrwl/nx 50
# Filter by publishedAt in the output (dates are in ISO format)
```

For each relevant release (stable + backports; skip pre-releases unless
specifically requested), the helper above already includes `body`, so you
can filter the array. If you need a specific tag explicitly:

```bash
gh_release_view nrwl/nx <tag>
```
...
gh_repo_view nrwl/cloud-infrastructure 2>/dev/null
```
...
# gh_api_paginate concatenates every page of /commits into one JSON array.
gh_api_paginate "repos/nrwl/cloud-infrastructure/commits?since=<START_DATE>" \
  | jq -r '.[] |
