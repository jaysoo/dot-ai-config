...
JSON files instead of hitting the GitHub API. Cache files include the `body`
field, so separate release-view calls are unnecessary.

```bash
# Read cached releases (falls back to live REST helper)
cat "$SCAN_DATA_DIR/releases/<owner>-<repo>.json" 2>/dev/null \
  || gh_release_list <owner>/<repo> 15
```
