...
- Feature-adoption stickiness: use MUTUALLY EXCLUSIVE buckets - overlapping cuts ("zero compute" includes AI-only orgs) hide the real pattern. Exclusive (tested 2026-07, 972 Team orgs): cache-only 4.4%, compute-only 2.4% (lowest; suggestive at n=3/127), AI-only 7.3%, both 11.1%. AI is the churn-correlated feature (2.3x, z~2.8); agents-only mildly protective. Possible confound: AI adopters may be distressed accounts (painful CI) - unresolved. Do not claim "drive adoption for retention" without spend-preview mitigations; do not push AI adoption as retention at all until the correlation is understood.
- UNITS: `computationTimeSavedMs` is TASK-time, not machine-time - runners execute 2+ tasks per machine-minute, so comparing $/saved-task-min against machine-minute runner rates (GHA $0.006) flatters ~2x. Use per-core rates (GHA $0.003, Blacksmith $0.002) as the conservative benchmark and present ranges. Cross-check savings via dailyTaskStatistics per-type math (remote + local components should reconcile with dailyProductUsage savedMs within ~5%).
