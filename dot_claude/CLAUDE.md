...
- **Polygraph `push_branch` cannot take an amended commit.** It runs `git pull --rebase`, so amending an already-pushed commit conflicts and leaves a mid-rebase. For follow-up changes on an open Polygraph PR, add a **new follow-up commit** (push_branch fast-forwards) instead of `git commit --amend`. Before ANY `--amend`, confirm the tip is unpushed: `git log origin/<branch>..HEAD` must show the commit (amended a pushed commit in DOC-549; pull-rebase duplicated it). Force-push via SSH only if you truly need one commit and have a remote to the target repo: logged `git fetch origin <branch> && git push --force-with-lease`, then `create_pr`/`mark_pr_ready` work normally. Also: a push_branch call the user rejected/interrupted may have ALREADY pushed - check the remote before assuming it didn't. Recurring (CLOUD-4612, #35922, ocean #11878) - don't relearn it each session. Also: a draft Polygraph PR you're iterating on can be **squash-merged + its branch deleted by a teammate MID-SESSION** - push_branch then silently RE-CREATES the branch and still reports success (and `show_session` may show a stale `MERGED`). Verify the real PR state via GitHub (WebFetch the PR URL) before trusting it; for post-merge follow-ups, branch off current `origin/master` and `git rebase --onto origin/master <last-merged-commit>` (the squashed foundation commits aren't ancestors, so a plain rebase re-applies + conflicts), then `create_pr` a fresh draft. (DOC-537 #36088 merged mid-session, follow-up #36105.) Also: Nx Cloud self-healing can push an empty `[Self-Healing CI Rerun]` commit to the PR branch mid-session; the next push_branch pull-rebase folds it in and rewrites local SHAs. Before assuming corruption, check `git diff <old-sha> <new-tip>` - empty diff means a harmless retrigger commit. (NXC-4179 #36408.)
- **Polygraph child agents in another repo deadlock on permission gates.** Every bash command the child runs opens an elicitation dialog only Jack can clear; the delegate subagent cannot resolve it (no `allow_agent`) and cannot deliver follow-up instructions while one is pending, so it stops and re-notifies. Two attempts on nrwl/ocean burned ~200k subagent tokens and returned nothing. For **read-only source questions** in another repo, ask Jack to run his own permissioned agent - that answered five questions thoroughly in one pass. Reserve `spawn_agent` for work that actually writes code in that repo. (2026-08-06 credit usage.)
- **Check the PR state before `--amend`, not just the branch.** `git ls-remote` proves the branch exists on the remote; it says NOTHING about whether the PR is still open. Run `gh pr view <n> --json state` first. This applies to plain `gh` PRs, not just Polygraph. (nx #36581, 2026-08-05: PR number was in the user's first message, four amend rounds followed, the PR squash-merged partway through, and only the first two reached master - nothing errored.) Recovery: branch off current `origin/master` and apply just `git diff origin/master HEAD -- <paths>`; never rebase the dead branch onto master (the squashed commit isn't an ancestor).
- **Verify the commit author after amending**: `git log -1 --format='%an <%ae>'`. A commit inherited from a prior session can be authored by `Claude <noreply@anthropic.com>` - fix with `git commit --amend --reset-author`. Later `--amend --no-edit` calls preserve the corrected author.
...
**Inline code comment style**: Terse. 1-2 lines max, explain _why_ not _what_. Delete comments that recap mechanics already visible in the code. Just solved something subtle? The investigation narrative goes in the commit body/PR, NOT an inline block - the comment keeps only the 1-2 line why. Bullets OK for a pipeline of transforms. JSDoc on public APIs may go longer for non-obvious contracts. When applying review feedback, do not transcribe the reviewer's rationale into the comment - the fix gets the same 1-3 line why as any other code (NXC-4687: 8-line comment cut 75% by Jack). Deliberate audit: `code-comment-style` skill.
...
- **Off-hand remarks are not roadmap.** Never state deprecation/removal/roadmap intent in PR bodies, comments, or docs from a passing chat remark ("we should remove X later"). Only write it if it's an actual plan — a Linear project/issue or explicit confirmation.
  - **Why:** NXC-4687 — PR body said "presets are slated for removal" from an aside; Jason questioned it in Slack and Jack had to walk it back.
...
- **`rg -r` is `--replace`, NOT recursive** (ripgrep recurses by default). `rg -rn "foo" apps libs` searches for `apps` inside `libs/` and prints replacements - so a null result is NOT evidence the term is absent. Hit twice on 2026-08-06, once via a subagent that concluded a field didn't exist. Also `rg -o '.{0,200}term.{0,200}'` blows ugrep's complexity limit on large files; slice context with a Node/Python one-liner instead.
- **Shell is fish**: inline `bash -c` / `python3 -c` / `node -e` strings mangle `!` and `!=` (history expansion + line-continuation), silently breaking comparisons (`r['status'] != 'done'` becomes a syntax error or no-op). Single-quoted heredocs (`python3 - <<'EOF'`) are NOT safe either - `\!=` still arrives mangled. Always write the script to a file (Write tool) and run the file. Alternatively `not in (...)` / `<>`. A background CI poller using `!=` polled blindly for 90 min and reported nothing - always confirm a long-running poller emits a real signal before trusting it. Also: `$status` after a pipeline reflects only the LAST stage - `git push ... | tail` reports `tail`'s exit (0) and masks a failed push; use `$pipestatus[1]` (or run the command unpiped). Once reported a failed force-push as success (NXC-4548). RULE: never pipe `git push` (or any must-not-fail command) - redirect to a file (`> /tmp/push.log 2>&1`), check `$status`, then read the file. Recurred in NXC-4687 despite this entry. Also: fish does NOT word-split variables - `set FILES "a.ts b.ts"; npx prettier $FILES` passes the whole string as ONE argument (fails "no files matching"); pass paths explicitly or use a real fish list (`set FILES a.ts b.ts`). Hit twice in Q-503. Also: fish CONTROL-FLOW breaks in the Bash tool - `test ...; and ...; or ...`, `if/else/end` inside a piped or compound command, and `mapfile` (bash-only) all error mid-command. For any loop/conditional/array work (renumbering, per-file edits, list filtering), write a Node `.mjs` script and run the file instead of inlining fish control-flow. Hit ~5x in DOC-537. Also: QUOTE glob patterns inside command flags - `grep --include='*.ts'`. Unquoted `--include=*.ts` dies with `(eval):1: no matches found` before the command runs, because fish globs the flag value against the cwd.
...
- **Don't stall on an external answer you can get locally.** Before asking another repo/agent/human for a schema, field list, or config, check whether the current repo already reads it - an existing query module, migration, fixture, or type. Ask only for what is genuinely unreachable. If the answer doesn't arrive, build against what IS known with tolerant handling plus a loud mismatch signal (log fields that came back empty), rather than pausing and re-asking.
  - **Why:** 2026-08-06 credit usage - asked ocean three times for the `MBillingRecord` shape while lighthouse already had a `Queries.BillingRecords` module doing an unprojected `find` that revealed the composite `_id.orgId` / `_id.periodStart` key. Burned a turn reporting "still blocked on the data class".
...
- **Snapshot state AT the event, not after.** When measuring "state at churn/incident/change", cap the time series before each entity's event date — a `$last`/latest-doc snapshot silently reflects post-event state.
  - **Why:** ocean Team-churn analysis (2026-07) — latest `dailyProductUsage` doc showed post-churn free-tier usage, not at-churn value; had to re-export the full daily series and cut per-org at churn date.
...
### Prod data analysis (billing/analytics)
