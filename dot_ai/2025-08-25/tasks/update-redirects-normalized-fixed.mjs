...
function normalizeAstroUrl(url) {
  ...
  const normalizedParts = parts.map(part => {
    ...
    // Remove other special characters (except dashes and underscores)
    normalized = normalized.replace(/[^a-z0-9\s\-_]/g, '');
    
    // Replace remaining spaces with single dashes
    normalized = normalized.replace(/\s+/g, '-');
    
    // Clean up multiple dashes (but preserve -- from & replacement)
    // Only collapse 3+ dashes to 2 dashes
...
// Write transformation log with examples
const transformLog = path.join(__dirname, 'url-transformations-fixed.txt');
let logContent = 'URL NORMALIZATIONS APPLIED (FIXED VERSION)\n';
logContent += '===========================================\n\n';

// Show some examples of & replacement
const ampersandExamples = urlTransformations.filter(t => t.original.includes('&')).slice(0, 10);
if (ampersandExamples.length > 0) {
  logContent += 'AMPERSAND (&) → DOUBLE DASH (--):\n';
  for (const t of ampersandExamples) {
    logContent += `  ${t.original}\n  → ${t.normalized}\n\n`;
  }
}

// Show total stats
logContent += 'TRANSFORMATION SUMMARY:\n';
logContent += `- Total URLs normalized: ${stats.urlsNormalized}\n`;
logContent += `- Ampersands replaced with --: ${urlTransformations.filter(t => t.original.includes('&')).length}\n`;
logContent += `- Uppercase to lowercase: ${urlTransformations.filter(t => t.original !== t.original.toLowerCase()).length}\n`;
logContent += `- Spaces replaced with -: ${urlTransformations.filter(t => t.original.includes(' ')).length}\n`;

fs.writeFileSync(transformLog, logContent);

// Generate summary
console.log('✅ Updated redirect file with CORRECTED normalized URLs');
console.log('\nURL Normalization Applied:');
console.log('- Lowercase: All URLs converted to lowercase');
console.log('- Ampersands (&): Replaced with double dash (--)');
console.log('- Spaces: Replaced with single dash (-)');
