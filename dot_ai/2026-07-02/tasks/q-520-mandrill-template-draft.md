...
- Members can request once per 48h per org+feature (server-enforced), so admins get at most one of these per window per add-on.
- Recipients: every org admin with a verified email (`Notifier.getOrganizationUsers(org, "admin")` filtered by `emailVerified`) - same recipient logic as `nx-cloud-plan-automated-add-ons-added`.
- Remember to PUBLISH the template after saving (drafts are not sendable via API).
