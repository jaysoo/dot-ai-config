#!/usr/bin/env node

/**
 * Comprehensive redirect test using Playwright
 * Tests all redirects and compares content with production
 * Generated: 2025-08-21
 */

import { chromium } from 'playwright';
import { readFileSync } from 'fs';
import { writeFileSync } from 'fs';

// Extract all redirects from the redirect rules file
function extractRedirects() {
  const fileContent = readFileSync('/Users/jack/projects/nx-worktrees/DOC-154/nx-dev/nx-dev/redirect-rules-docs-to-astro.js', 'utf8');
  const redirects = {};
  
  // Match all redirect patterns
  const pattern = /'([^']+)': '([^']+)'/g;
  let match;
  
  while ((match = pattern.exec(fileContent)) !== null) {
    if (match[1].startsWith('/') && match[2].startsWith('/docs')) {
      redirects[match[1]] = match[2];
    }
  }
  
  return redirects;
}

async function testRedirects() {
  const redirects = extractRedirects();
  const totalRedirects = Object.keys(redirects).length;
  console.log(`Found ${totalRedirects} redirects to test\n`);
  
  const browser = await chromium.launch({ headless: true });
  const context = await browser.newContext();
  
  const results = {
    success: [],
    failed404: [],
    failedNoRedirect: [],
    contentMismatch: [],
    errors: []
  };
  
  let testCount = 0;
  
  // Test each redirect
  for (const [oldPath, expectedNewPath] of Object.entries(redirects)) {
    testCount++;
    process.stdout.write(`Testing ${testCount}/${totalRedirects}: ${oldPath}`);
    
    try {
      // Create pages for local and production tests
      const localPage = await context.newPage();
      const prodPage = await context.newPage();
      
      // Test local redirect
      const localUrl = `http://localhost:4200${oldPath}`;
      const localResponse = await localPage.goto(localUrl, { 
        waitUntil: 'networkidle',
        timeout: 10000 
      });
      
      const finalLocalUrl = localPage.url();
      const localStatus = localResponse.status();
      
      // Check if redirect happened correctly
      if (localStatus === 404) {
        results.failed404.push({
          path: oldPath,
          expected: expectedNewPath,
          status: localStatus
        });
        process.stdout.write(' ❌ 404\n');
        await localPage.close();
        await prodPage.close();
        continue;
      }
      
      // Check if URL was redirected to expected path
      const expectedUrl = `http://localhost:9001${expectedNewPath}`;
      if (!finalLocalUrl.includes(expectedNewPath.replace('/docs', ''))) {
        results.failedNoRedirect.push({
          path: oldPath,
          expected: expectedNewPath,
          actual: finalLocalUrl
        });
        process.stdout.write(' ❌ Wrong redirect\n');
        await localPage.close();
        await prodPage.close();
        continue;
      }
      
      // Get production content for comparison
      const prodUrl = `https://nx.dev${oldPath}`;
      try {
        await prodPage.goto(prodUrl, { 
          waitUntil: 'networkidle',
          timeout: 15000 
        });
        
        // Extract main content (title and first paragraph)
        const prodTitle = await prodPage.evaluate(() => {
          const h1 = document.querySelector('h1');
          return h1 ? h1.textContent.trim() : '';
        });
        
        const localTitle = await localPage.evaluate(() => {
          const h1 = document.querySelector('h1');
          return h1 ? h1.textContent.trim() : '';
        });
        
        // Basic content comparison
        if (prodTitle && localTitle && prodTitle !== localTitle) {
          // Check if titles are semantically similar (allow for minor differences)
          const titleSimilar = localTitle.toLowerCase().includes(prodTitle.toLowerCase().split(' ')[0]) ||
                              prodTitle.toLowerCase().includes(localTitle.toLowerCase().split(' ')[0]);
          
          if (!titleSimilar) {
            results.contentMismatch.push({
              path: oldPath,
              redirectedTo: expectedNewPath,
              prodTitle,
              localTitle
            });
            process.stdout.write(' ⚠️  Content mismatch\n');
          } else {
            results.success.push({
              path: oldPath,
              redirectedTo: expectedNewPath
            });
            process.stdout.write(' ✅\n');
          }
        } else {
          results.success.push({
            path: oldPath,
            redirectedTo: expectedNewPath
          });
          process.stdout.write(' ✅\n');
        }
        
      } catch (prodError) {
        // Production page might not exist (for new content)
        results.success.push({
          path: oldPath,
          redirectedTo: expectedNewPath,
          note: 'Production page not accessible'
        });
        process.stdout.write(' ✅ (prod N/A)\n');
      }
      
      await localPage.close();
      await prodPage.close();
      
    } catch (error) {
      results.errors.push({
        path: oldPath,
        error: error.message
      });
      process.stdout.write(` ❌ Error: ${error.message}\n`);
    }
  }
  
  await browser.close();
  
  // Generate report
  console.log('\n' + '='.repeat(80));
  console.log('REDIRECT TEST SUMMARY');
  console.log('='.repeat(80));
  
  console.log(`\n✅ Successful redirects: ${results.success.length}`);
  console.log(`❌ 404 errors: ${results.failed404.length}`);
  console.log(`❌ Wrong redirects: ${results.failedNoRedirect.length}`);
  console.log(`⚠️  Content mismatches: ${results.contentMismatch.length}`);
  console.log(`❌ Errors: ${results.errors.length}`);
  
  // Show details of failures
  if (results.failed404.length > 0) {
    console.log('\n404 ERRORS:');
    results.failed404.forEach(item => {
      console.log(`  ${item.path} → Expected: ${item.expected}`);
    });
  }
  
  if (results.failedNoRedirect.length > 0) {
    console.log('\nWRONG REDIRECTS:');
    results.failedNoRedirect.forEach(item => {
      console.log(`  ${item.path}`);
      console.log(`    Expected: ${item.expected}`);
      console.log(`    Actual: ${item.actual}`);
    });
  }
  
  if (results.contentMismatch.length > 0) {
    console.log('\nCONTENT MISMATCHES (may be acceptable):');
    results.contentMismatch.forEach(item => {
      console.log(`  ${item.path}`);
      console.log(`    Prod title: "${item.prodTitle}"`);
      console.log(`    Local title: "${item.localTitle}"`);
    });
  }
  
  if (results.errors.length > 0) {
    console.log('\nERRORS:');
    results.errors.forEach(item => {
      console.log(`  ${item.path}: ${item.error}`);
    });
  }
  
  // Save detailed report
  const reportPath = '/Users/jack/projects/nx-worktrees/DOC-154/.ai/2025-08-21/tasks/redirect-test-report.json';
  writeFileSync(reportPath, JSON.stringify(results, null, 2));
  console.log(`\nDetailed report saved to: ${reportPath}`);
  
  // Return exit code based on critical failures
  const criticalFailures = results.failed404.length + results.failedNoRedirect.length + results.errors.length;
  if (criticalFailures > 0) {
    console.log(`\n❌ Test failed with ${criticalFailures} critical issues`);
    process.exit(1);
  } else {
    console.log('\n✅ All redirects working correctly!');
    process.exit(0);
  }
}

// Check if servers are running
async function checkServers() {
  try {
    const browser = await chromium.launch({ headless: true });
    const page = await browser.newPage();
    
    // Check Astro server
    try {
      await page.goto('http://localhost:9001', { timeout: 5000 });
      console.log('✅ Astro server running on port 9001');
    } catch {
      console.log('❌ Astro server not running on port 9001');
      console.log('Please run: npx astro dev --port 9001');
      await browser.close();
      return false;
    }
    
    // Check nx-dev server
    try {
      await page.goto('http://localhost:4200', { timeout: 5000 });
      console.log('✅ nx-dev server running on port 4200');
    } catch {
      console.log('❌ nx-dev server not running on port 4200');
      console.log('Please run: NEXT_PUBLIC_ASTRO_URL=http://localhost:9001 nx serve nx-dev');
      await browser.close();
      return false;
    }
    
    await browser.close();
    return true;
  } catch (error) {
    console.error('Error checking servers:', error);
    return false;
  }
}

// Main execution
async function main() {
  console.log('Comprehensive Redirect Test');
  console.log('=' .repeat(80));
  
  const serversReady = await checkServers();
  if (!serversReady) {
    console.log('\nPlease start both servers and try again.');
    process.exit(1);
  }
  
  console.log('\nStarting redirect tests...\n');
  await testRedirects();
}

main().catch(console.error);