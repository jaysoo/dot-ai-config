# nx.dev GA Traffic Pipeline

Deterministic data-processing + visualization pipeline for nx.dev Google Analytics
traffic. Turns scraped raw GA4 JSON into merged chart data, a deterministic
analysis, and a self-contained chart app.

## Pipeline

1. **Scrape (manual, already done).** GA4 has no usable API here (needs auth, and
   Explore caps retention at ~14 months), so the series were pulled by hand via
   Chrome DevTools against the GA4 UI (standard Reports for Jan-Apr 2025, Explore
   free-form for May 2025 onward) and saved as `raw/*.json`. These are the source
   of truth and are never modified.
2. **Process.** Run `node process.mjs` from this directory. It reads `raw/*`,
   builds a merged daily table, computes the analysis, and writes `out/*`.
3. **View.** Open `out/chart.html` in a browser (works via `file://`; only the
   Chart.js CDN libs need network).

```bash
cd /Users/jack/projects/dot-ai-config/dot_ai/2026-06-19/tasks/ga-traffic
node process.mjs
open out/chart.html
```

`process.mjs` is a single Node ESM script with no external npm deps (only
`fs`, `path`, `url`). It is deterministic: same raw inputs -> identical outputs.

## Raw inputs (`raw/`)

| File | Contents |
| --- | --- |
| `monthly-segments.json` | 19 months Jan 2025..Jul 2026 (Jul partial, through Jul 29). Per month: all/home/docs/non-docs views, active users, server_page_view. |
| `daily-all.json` | 455 days through 2026-07-29. All-pages daily `views` (client page_view), active_users, event_count_all. (Jun 19 onward: views only; active_users/event_count null - not analyzed.) |
| `daily-home.json` | 455 days through 2026-07-29. Daily views/active_users for pagePath `/`. (Jun 19 onward: views only.) |
| `daily-docs.json` | Daily views/active_users for pagePath begins-with `/docs` (through 2026-07-29). ~0 before Sep 2025; `first_recorded` 2025-05-13. |
| `daily-blog.json` | 455 days through 2026-07-29. Daily views/active_users for pagePath begins-with `/blog`. (Jun 19 onward: views only.) |
| `daily-marketing.json` | 455 days through 2026-07-29. Daily views/active_users for the Framer marketing pages (pagePath matches the regex in `segment-definitions.json`). (Jun 19 onward: views only.) |
| `daily-server.json` | Days where `server_page_view` event > 0 (through 2026-07-29; consent-immune, so refreshes are meaningful here). `first_recorded` 2026-02-02. |
| `daily-server-by-category.json` | 178 days through 2026-07-29. Per (date, category) server_page_view: home/docs/blog/marketing/total, each with a `*_bf` companion = the `Bot` dimension = false (excludes flagged bots; bot classification only exists from 2026-02-18). Feeds the weekly server-by-category chart and its include/exclude-bots toggle. |
| `daily-nxdev-by-category.json` | 455 days through 2026-07-29. Same MECE categories as the client dailies but filtered to `Hostname = nx.dev` EXACTLY (excludes `*.nx.dev` versioned subdomains, `go.*`, `canary`, `localhost`, and Vercel/Netlify preview deploys). Backs the top-of-page "nx.dev hostname only" toggle. |
| `server-human-signal.json` | Documents the (failed) attempt to isolate a HUMAN server signal via the `Bot`+`AI Tool` dimensions: even `Bot=false AND AI-Tool=false` grows ~2-4x Feb->Apr, so GA cannot cleanly separate humans from undetected AI crawlers. Reference only (not charted). |
| `top-pages.json` | Top-25 pages per category over the full daily window (Views). Note Others is dominated by OLD root-path docs that pre-date the 2025-09-29 /docs move. |
| `top-pages-before-after.json` | Top pages per category BEFORE (2025-05..09-28) vs AFTER (2025-09-29..) the astro-docs /docs switch. Shows root-path docs leaving Others (1.97M -> 100K) and reappearing as /docs (0 -> 2.25M). Renders the before/after section in chart.html. |
| `segment-definitions.json` | The MECE page-type rules: home (`/`), docs (`/docs*`), blog (`/blog*`), marketing (Framer-page regex), others (= all - the rest). Documents coverage (blog/marketing daily from May 2025). |
| `events.json` | 10 config/platform/tracking events with dates and expected impact: GTM added, astro-docs /docs live (2025-09-29), server_page_view start (2026-02-02), Vercel->Netlify DNS cutover (2026-02-04), GTM-only cutover (#34384, 2026-02-10), Framer migration (2026-02-27), nx-blog standalone (2026-04-16), nx-dev pared down (2026-04-17), bot-probe 404s (2026-04-30), Cookiebot banner (2026-05-01). |
| `source-crosschecks.json` | Reports-vs-Explore agreement checks (May and Aug 2025 match exactly). Loaded by `process.mjs`, which recomputes them deterministically (Reports == Explore == monthly-segments) and emits `analysis.source_crosscheck`. |
| `gsc-daily.json` | **Google Search Console** daily web-search CLICKS + impressions for `sc-domain:nx.dev` (2025-02-25..2026-07-29; GSC's 16-month retention). CONSENT-INDEPENDENT organic signal - GSC counts Google-side clicks, immune to the Cookiebot banner that suppressed GA4 client page_view from 2026-05-01. The decisive cross-check the predecessor flagged: confirms the organic decline is real and stays measurable across the May banner. Backs the organic cross-check chart and `analysis.organic`. Scraped from GSC > Performance > Search results > DAYS, Last 16 months. |
| `gsc-docs-others-daily.json` | **GSC daily CLICKS for the DOCUMENTATION** (docs + others = all pages EXCEPT home/blog/marketing; `others` pre-2025-09-29 IS the root-path docs, so it carries the doc-organic story across the astro-docs move). 485 days, consent-independent. Page filter = "does NOT match regex" for home(`/`)/blog/marketing (the Framer page list from `segment-definitions.json`). Total 1,532,155 matches the GSC filtered header. Backs the **weekly docs-organic forward tracker** chart and `analysis.organic.docs_forward`. THE metric to watch for SEO/AI-search correction progress. |
| `channels-by-month.json` | Monthly client Views by Session default channel group (organic_search/direct/referral/unassigned/other) for home/docs/others + `*Nxdev` variants, 2025-05..2026-06. Backs the per-category "traffic by source" charts and the GA4-organic side of `analysis.organic`. |

`monthly-segments.json` is generated by `raw/_gen-monthly.mjs` from the verified
scrape array in `../ga-monthly-traffic.mjs` (run `node raw/_gen-monthly.mjs` to
reproduce it byte-for-byte). The daily `*.json` files are hand-scraped raw sources
of truth and have no generator.

`active_users` (daily and monthly) and `daily-all.json`'s `event_count_all` are
retained in the raw inputs but intentionally NOT analyzed or charted: AU is
contaminated by server-side MP events from Feb 2026 (not comparable across that
boundary), and event_count_all mixes client and server events. They are kept for
completeness only.

## Outputs (`out/`)

| File | Contents |
| --- | --- |
| `chart-data.json` | `{ meta, events, daily[], weekly[], weeklyNxdev[], weeklyServerCat[], monthly[], monthlyNxdev[], topPages, topPagesBeforeAfter }`. Each client row carries the MECE page-type segments (home/docs/blog/marketing/others views; others = all - the rest) + server_page_view. `*Nxdev` = the `Hostname=nx.dev` variant; `weeklyServerCat` = server_page_view by category with `*_bf` (bot=false) companions. |
| `analysis.json` | Deterministic computations: `mom` (month-over-month % for all/home/docs), `trend` (last 12 full months, OLS slope + total %, plus a `pre_banner` parallel trend through 2026-04 since the 12-month endpoint is consent-suppressed), `organic` (the consent-independent organic story: GSC clicks vs GA4 organic Views, baseline-window Oct2025->Apr2026 change for both, a `cross_check` that proves the decline is real AND that May is the banner, `forward_tracking` = the site-wide GSC-clicks baseline/latest, and `docs_forward` = the DOCS-specific forward tracker (docs+others; GSC clicks weekly + GA4 organic Views monthly, baseline-window change, May divergence, GSC forward baseline) backing the weekly docs-organic chart), `event_impacts` (14d-before/14d-after window on daily all_views per event; the after-window is inclusive of the event day, surfaced in `after_window`, and a `reason` is added when the before-window is truncated by the daily span start), `cookiebot_2026_05_01`, `server_page_view` (first/peak/monthly totals), `daily_vs_monthly_check` (per-series integrity for all/home/docs/server: monthly vs sum-of-daily, tolerance flag), `source_crosscheck` (recomputed Reports == Explore == monthly-segments agreement), `key_findings`. |
| `chart.html` | Self-contained chart app. Data is injected inline by `process.mjs` (not fetched), so it opens via `file://`. Top-of-page **"nx.dev hostname only" toggle** switches the client charts to the canonical-host series. THREE time-axis STACKED-AREA charts: (1) WEEKLY client Views by page type (full ISO weeks), (2) MONTHLY client Views back to Jan 2025 - both with all 10 milestones as annotated vertical lines + `server_page_view` as an orange right-axis line; (3) WEEKLY `server_page_view` by page type with an **include/exclude-bots toggle**. The white "Total (all types)" line recomputes to the sum of VISIBLE categories when you toggle legend items off. Below the charts: key findings, top-25 pages per category, and a **before/after the 2025-09-29 /docs switch** comparison. `CATEGORY-TRENDS-REPORT.md` (sibling file) is the written per-category MoM + steepest-decline + internal/external correlation analysis. |

## Key caveats

- **Views = client page_view** (GA4 `screenPageViews`). That is the only metric
  called "Views" here.
- **server_page_view is a separate event** (Measurement Protocol, server-side),
  NOT counted in Views, and is mostly AI-crawler traffic. It sits on its own
  right-hand axis.
- **No /2 normalization.** The double-count theory is disproven: pre-GTM
  (Jan-Apr 2025, ~650K/mo) matches post-GTM (May-Jul 2025, ~635K). Raw Views are
  directly comparable across the whole range.
- **The big May 2026 drop is the Cookiebot consent banner** (2026-05-01), a
  measurement effect (consent-gated client GA), not lost traffic - server_page_view
  stays flat/growing across it. There is also a real, separate pre-banner decline
  in Mar-Apr 2026. Because of this, the 12-month `trend` endpoint (2026-05) is
  consent-suppressed: read `trend.pre_banner` (through 2026-04) for the organic
  signal (~-52% all_views, ~-33% home), and treat the headline 12-month % as
  inflated by the measurement cliff. The home trend in particular is flat (~54K/mo)
  through Apr 2026 then a single May cliff, not a gradual decline.
- **Active users (AU) is contaminated** by server-side MP events from Feb 2026, so
  AU before vs after Feb 2026 is not comparable. The analysis does not trend AU.
- **The docs / non-docs split is only meaningful from ~Oct 2025**, when astro-docs
  moved to `/docs`. `process.mjs` zeroes pre-launch `/docs` Views (before 2025-09-29)
  into Others - those were 61 stray bot/scanner hits on URLs that did not exist yet.
- **Hostname matters.** The GA property tracks `*.nx.dev` versioned subdomains, redirect
  domains, `localhost`, and preview deploys alongside `nx.dev`. The non-nx.dev share is
  time-varying (2.5%-22%/mo, peak Jan 2026), so the all-hosts view overstates some of the
  late-2025/early-2026 numbers. The "nx.dev hostname only" toggle strips them.
- **The Bot/AI-Tool dimensions cannot isolate humans.** server_page_view is ~30% flagged-bot
  and ~7% flagged-AI-Tool, but even `Bot=false AND AI-Tool=false` grows ~2-4x Feb->Apr -
  undetected AI crawlers evade both flags. So server-side GA cannot prove the client decline
  is real vs measurement (see `server-human-signal.json`).
- **Decline read (calibrated).** The Mar-Apr 2026 pre-banner decline is content-agnostic:
  home/docs/blog all fell ~-33 to -43% (Oct 2025->Apr 2026) while marketing GREW +39 to +52%.
  Three unrelated page types declining by the same % is the signature of a shared client-capture
  regression (the GTM-only cutover #34384 making page_view depend on the GTM container config),
  not content-specific loss. Likely measurement-led with a smaller real AI/organic residual.
  NOTE: not an ad-blocking effect - gtm.js and gtag/js are blocked equally.
- **GSC page-regex filter is contaminated by AI data (caught 2026-06-29).** As of ~late June 2026, GSC's
  Search-Console "Web" data now surfaces AI Overview / AI Mode clicks+impressions, but ONLY in the
  page-FILTERED view, not the unfiltered property total. Effect: the docs+others regex filter returned
  MORE clicks (2.02M) than the entire unfiltered site (1.88M) - a subset exceeding the whole, which is
  impossible. Impressions ~tripled in the filtered view (CTR 3.8% unfiltered -> 1.6% filtered). So the
  `gsc-docs-others-daily.json` regex breakdown is UNRELIABLE until Google's rollout stabilizes; it is held
  at 2026-06-24. The UNFILTERED site-wide `gsc-daily.json` is UNCHANGED and reliable (verified Jun16-24
  overlap exact) - use it as the doc-organic proxy (nx.dev is ~80% docs). LESSON: a subset filter reading
  higher than the unfiltered total is the tell; always check subset vs whole before calling it a reprocess.
- **Data currency (2026-07-30 update):** ALL GA4 dailies (all/home/docs/blog/marketing/nxdev/server/server-by-category),
  site-wide GSC (`gsc-daily.json`), monthly-segments (Jun 2026 finalized, Jul 2026 partial thru Jul 29), and
  channels-by-month refreshed through 2026-07-29. docs+others GSC still held at 2026-06-24 (see above).
  GA4 May-Jul stay consent-suppressed. Scrape method: GA4 internal report RPC replayed in-page (see
  `dot_ai/2026-07-30/tasks/ga-traffic-refresh/`); overlap days re-verified, only 1-3-view restatements on 2 days.
  July organic: GSC clicks 49,259 thru Jul 29 (~52.7K full-month pace) vs June 59,022 - the organic decline continues
  ~-11%/mo post-banner. server_page_view holds ~5.1-5.4M/mo; server-side blog grew Apr->Jun (0.8M -> 1.7M/mo, now ~30% of server events)
  and is roughly flat in July (~1.57M full-month pace), docs steady at ~2.4M/mo.
- **Organic decline is REAL (resolved 2026-06-26).** The decisive test - GA4 page_views vs Google
  Search Console CLICKS (consent-independent) - is now run (`raw/gsc-daily.json`). GSC organic clicks
  fell -40.6% Oct 2025 -> Apr 2026, independently corroborating GA4 organic Views -45.6% (docs -46.3%)
  over the same window: the organic decline is real, not a measurement artifact. AND the two signals
  DIVERGE across May 2026 (GSC -8.5% vs GA4 organic -69.6%), proving the May cliff is the Cookiebot
  banner (measurement), not lost audience. GSC clicks is consent-immune, so it is the canonical
  FORWARD-TRACKING metric for SEO/AI-search correction efforts (baseline May 2026 = 66,383 clicks/mo).
