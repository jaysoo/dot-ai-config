...
|------|------|-------------------|
| 2026-07-29 | [cve-audit-2026-07-29.md](./cve-audit-2026-07-29.md) | First full lockfile audit of all 4 repos (`pnpm`/`bun`/`yarn audit` against real lockfiles, not just package.json ranges). 4 findings resolved/mooted (ocean `tar` client-bundle + MCP SDK, nx-labs `storybook`, nx-console migrated off vulnerable MCP SDK v1). nx `rollup`/`vite` now **~49 days stale**, still unfixed. New: nx-labs `vm2` sandbox-escape chain (CRITICAL, dev-tool only), ocean `path-to-regexp`/express, nx-console stale internal `@nx/*` beta pins pulling vulnerable minimatch |
