...
|------|------|-------------------|
| 2026-08-01 | [cve-audit-2026-08-01.md](./cve-audit-2026-08-01.md) | Widened methodology: full `pnpm`/`bun`/npm-bulk-advisories sweep of each repo's actual lockfile (not just hand-traced known packages), surfacing many more transitive findings. Published `nx@23.1.1` + 19 first-party plugins re-verified fully clean. Ocean resolved 4 of 6 open items (`tar`, MCP SDK, OTel/gRPC bundle, `undici`); nx-labs resolved `storybook`. nx `rollup`/`vite` now **52 days stale**. New: `vm2` in nx-labs (19 advisories, most CRITICAL, package is abandoned — no version fix exists) |
