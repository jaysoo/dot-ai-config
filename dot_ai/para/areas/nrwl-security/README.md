...
|------|------|-------------------|
| 2026-07-23 | [cve-audit-2026-07-23.md](./cve-audit-2026-07-23.md) | Delta audit, re-verified against actual lockfiles (not just declared ranges). 5 findings resolved: ocean `tar` + `@modelcontextprotocol/sdk`, nx-labs `storybook` (incidental), nx-console `@modelcontextprotocol/sdk` + `minimatch`. nx `rollup`/`vite` now **43 days stale**, still no fix — escalated again. nx-console `express`/`path-to-regexp` fix conspicuously skipped in the same commit that fixed its two sibling findings |
