...
|------|------|------|-------|
| 2026-07-23 | nrwl/nx | [nx-astro-docs-staleness-2026-07-23.md](./nx-astro-docs-staleness-2026-07-23.md) | Targeted 3-smell pass. Live npm check revealed Nx 23.1.0 shipped 2026-07-13 (10 days prior) — `reference/releases.mdoc`'s supported-versions table is now stale (still labels v22 "Current", still lists v20 as supported LTS past its own 18-month policy window). 4 more new mismatched-feature findings: missing `buildTargets` option (eslint-plugin enforce-module-boundaries), missing `compiler` option + incomplete `serveTargetName` deprecation note (vite introduction), missing 4 options (typescript introduction), legacy `start-storybook`/`build-storybook` executor names (storybook overview-angular, a 3rd distinct issue on an already-tracked file). Backlog now 32 Linear issues queued — MCP still unavailable (7th cycle; symptom changed again, now `enabledInChat: false` after last cycle claimed `true`). |
