...
|------|------|------|-------|
| 2026-07-13 | nrwl/nx | [nx-astro-docs-staleness-2026-07-13.md](./nx-astro-docs-staleness-2026-07-13.md) | Full sweep of all 511 mdoc files (10 parallel agents fanning out into ~35 leaf audits) — broadest pass since 06-29, same 3-smell scope as 07-10. Re-verified 6 prior backlog items (still unfixed: #5, #7, #10, #13, #22 among them); 24 new issues found. Biggest theme: deprecated generators/executors (Module Federation host/remote in both React and Angular docs, Cypress, Rollup, ESLint, React Native, Storybook addons) presented as current with zero deprecation notice. Also found: reference/releases.mdoc version table stale (v22 shown as "Current"), nx-json.mdoc documenting fake root properties, changelog-renderer custom-class example that would fail to import. Backlog now 51 Linear issues queued — MCP still unavailable (7th cycle in a row, same symptom as 07-10: connector shows enabled but zero tools exposed — recommend investigating connector auth directly instead of retrying). |
