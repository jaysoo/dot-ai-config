...
|------|------|------|-------|
| 2026-08-07 | nrwl/nx + nrwl/ocean | [nx-astro-docs-staleness-2026-08-07.md](./nx-astro-docs-staleness-2026-08-07.md) | Targeted 4-smell pass, first cycle with `nrwl/ocean` read access — cross-checked `kb/github-app-permissions.mdoc` against real GitHub App call sites (clean, no drift) and `reference/nx-cloud-cli.mdoc` against ocean's actual command registry (**~12 commands missing/uncross-linked, largest finding to date**). Re-verified 4 prior findings still unfixed (publish-rust-crates.mdoc, v1-nx-plugin-api.mdoc + legacy-cache.mdoc future tense, terminal-ui.mdoc stale Windows notice); upgraded launch-template-examples.mdoc Node 21 finding from Needs-Input to Confirmed. Backlog now 29 Linear issues queued — MCP still unavailable (**7th cycle in a row, 3rd distinct symptom**: connector shows `connected` but `enabledInChat: false` this time. Recommend checking the connector toggle directly rather than waiting on another cycle). |
