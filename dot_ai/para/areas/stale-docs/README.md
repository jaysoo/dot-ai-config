...
|------|------|------|-------|
| 2026-07-05 | nrwl/nx | [nx-astro-docs-staleness-2026-07-05.md](./nx-astro-docs-staleness-2026-07-05.md) | Sixth scan; full pass over all 503 mdoc files with 13 parallel section agents, each cross-verified against `packages/` source and (for CI claims) the repo's own `.github/workflows/*.yml`. Big new findings: Module Federation guides (8 files) teach generators removed in Nx v24 with no warning; the astro-docs doc-generator script silently drops `x-deprecated` schema annotations site-wide (root-cause/tooling bug); extending-nx guides teach the deprecated `createNodesV2` API; `releases.mdoc` support table stale (v22 shown Current, v23 missing); `nx-json.mdoc` documents the wrong Nx Cloud encryption-key property name. 25 Linear issues queued — MCP still unavailable (5th consecutive audit with this blocker; worth escalating separately). Several 06-29 findings (publish-rust-crates.mdoc, nx-daemon.mdoc, setup-node@v3, launch-template-examples.mdoc Node 21) confirmed still open/unfixed. |
