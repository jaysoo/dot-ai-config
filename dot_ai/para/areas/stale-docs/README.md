...
|------|------|------|-------|
| 2026-07-09 | nrwl/nx | [nx-astro-docs-staleness-2026-07-09.md](./nx-astro-docs-staleness-2026-07-09.md) | Sixth scan; corrected baseline via live `npm view nx version` (23.0.1 is current stable — the 06-29 scan's "22.x is current" assumption was wrong, confirming the postmortem's own warning). Key findings: Module Federation guides teach deprecated host/remote generators as current with no pointer to consumer/provider; nx-json.mdoc misplaces task options at wrong nesting level; project-configuration.mdoc shows broken pre-v16 dependsOn syntax; sandboxing.mdoc documents `--inputs`/`--outputs` as flags when they're subcommands; Storybook guides describe v8-only addon behavior as universal default. GitHub Actions version claims marked needs-input (repo scope prevented external verification). Linear MCP unavailable for the 6th consecutive scan — 19 more issues queued, ~44 cumulative queued issues across all scans may never have been filed; flagged for a human to check the connector directly instead of relying on further automated retries. |
