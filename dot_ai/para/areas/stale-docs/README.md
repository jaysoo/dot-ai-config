...
|------|------|------|-------|
| 2026-07-29 | nrwl/nx | [nx-astro-docs-staleness-2026-07-29.md](./nx-astro-docs-staleness-2026-07-29.md) | Full 5-agent sweep of all 480 mdoc files, plus a full remap of the prior 46-item backlog. **Docs site was restructured this cycle** — extending-nx/, guides/Nx Cloud/, guides/Tips-n-Tricks/, and technologies/*/Guides|concepts/ mostly flattened into a new `kb/` dir (184 files); 2 files deleted outright with no replacement found. 9 prior findings resolved/fixed (several simply overtaken by Nx reaching 23.1.0, as predicted last cycle); 2 new High findings (nx-json.mdoc documents nonexistent/misplaced root fields; nx-daemon.mdoc useDaemonProcess location wrong). Backlog now 35 Linear issues queued. **Linear MCP root cause finally diagnosed** (7th cycle unavailable): `ListConnectors` shows the connector is `connected: true` at the org level but `enabledInChat: false` for this session — a toggle only Jack can flip, not a broken integration. |
