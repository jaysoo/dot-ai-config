...
|------|------|------|-------|
| 2026-07-02 | nrwl/nx | [nx-astro-docs-staleness-2026-07-02.md](./nx-astro-docs-staleness-2026-07-02.md) | Sixth scan; repo baseline moved 22.x→23.1.0-beta.4, resolving 06-29's H-1 (compat tables now correctly show "23.x (current)") — H-2/H-3/H-4/NI-1/NI-2 need re-verification, not yet closed. New findings: `nx generate`/`nx exec` flags entirely undocumented (hardcoded out of the CLI-reference generator), `buildDepsTargetName`/`watchDepsTargetName` missing from 4 plugin option tables (next/remix/webpack/storybook — systemic), vite missing `compiler` option, nx-json.mdoc missing 3 top-level properties. 7 new Linear issues queued (27–33) plus a re-verify follow-up on 06-29 #1–3. Linear MCP unavailable for the 6th straight run — see process note in the report; ~33 issues never actually filed. |
