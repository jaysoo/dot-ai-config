...
|------|------|------|-------|
| 2026-07-06 | nrwl/nx | [nx-astro-docs-staleness-2026-07-06.md](./nx-astro-docs-staleness-2026-07-06.md) | Full scan of all 503 mdoc files, ~30 parallel agents, every claim cross-checked against `packages/` source. Live-verified baseline via `npm view nx dist-tags`: v23.0.1 is actually current (v22 is now LTS) — the docs' own releases.mdoc still says v22 is current, which is itself a confirmed finding. GH Actions versions live-verified too (checkout v7, setup-node v6, docker/login-action v4) — docs are 2-3 majors behind everywhere. 38 issues total (21 high), ~10 recurring/unfixed from 2026-06-29 (compose-executors, migration-generators, publish-rust-crates, terminal-ui Windows note, Storybook deprecated packages, bundling-node-projects node18, eslint flat-config trivia). New: nx.json reference has 2 confirmed wrong facts (root-level task options, max-cache-size cap), Module Federation docs (7 files) still lead with generators deprecated for v24 removal, createNodesV2 shown as current when deprecated, several non-existent generators/flags (@nx/angular:service, --project on migration generator, nx show target --inputs/--outputs). Linear MCP still unavailable — 38 issues queued for manual creation. |
