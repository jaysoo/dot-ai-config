...
|------|------|------|-------|
| 2026-08-04 | nrwl/nx | [nx-astro-docs-staleness-2026-08-04.md](./nx-astro-docs-staleness-2026-08-04.md) | Targeted 3-smell pass, not a full 478-file sweep. 4 new confirmed findings: `reference/releases.mdoc` missing the Nx 23 row entirely (own canonical support-status page a full major behind), stale "Vite parity in progress" note in ci-deployment.mdoc, invalid `@storybook/vue-vite` enum documented in storybook/introduction.mdoc, self-contradictory Nx 22/23 release-tag aside in nx-json.mdoc. Re-verified 3 still-open carryovers (v1-nx-plugin-api.mdoc future tense, access-tokens.mdoc stale framing, launch-template-examples.mdoc Node 21 example — promoted to confirmed). Backlog now 32 Linear issues queued — MCP still unavailable (**7th cycle in a row**). Root cause finally pinned down precisely this cycle: `ListConnectors` shows Linear `installState: "connected"` but `enabledInChat: false` for this session — connector is authenticated and working, just toggled off for chat access. Needs a human to flip it on (or reconfigure where this scheduled task runs) — cannot self-resolve. |
