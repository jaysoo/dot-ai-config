...
|------|------|------|-------|
| 2026-07-15 | nrwl/nx | [nx-astro-docs-staleness-2026-07-15.md](./nx-astro-docs-staleness-2026-07-15.md) | Full re-scan of all 511 mdoc files (most exhaustive since 06-29), ~17 parallel agents each cross-referencing prose against live `packages/` source. 53 confirmed findings (44 new, 9 re-verified still-unfixed from the backlog), 18 needs-input. Highlights: invalid `--unitTestRunner=vitest` in the Angular getting-started tutorial (breaks a copy-pasted command), `createNodesV2` taught as primary API when it's deprecated in favor of `createNodes`, Angular + React Module Federation generators deprecated but undocumented across 12 guides, `nx-json.mdoc` root-property table wrong, flat `releaseTagPattern*` docs don't mention it now hard-errors on Nx 23. Backlog now **63 Linear issues queued** — MCP still unavailable (**7th cycle in a row**, zero Linear tools exposed despite connector showing enabled; recommend investigating the connector directly rather than retrying again next cycle). |
