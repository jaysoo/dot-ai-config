...
|------|------|------|-------|
| 2026-08-01 | nrwl/nx | [nx-astro-docs-staleness-2026-08-01.md](./nx-astro-docs-staleness-2026-08-01.md) | Full sweep of all 478 mdoc files (6 parallel agents), targeted at the same 3 smells. 0 old-Nx/old-Node/old-framework findings — `technologies/` and `guides/` are clean. 15 confirmed option/feature-drift findings + 1 needs-input, mostly in `kb/` (deprecated Storybook packages/addons, wrong `externalDependencies` default, dead `@nx/vite:test` executor, invalid Cypress executor option, invalid `--unitTestRunner=vitest`, non-existent `nx show target --inputs/--outputs` flags, stale `releases.mdoc` version table, `nx-json.mdoc`/`environment-variables.mdoc` drift). **Discovered astro-docs was restructured** since 07-10 — `extending-nx/` and `technologies/test-tools/storybook/Guides/` are now thin index shells; that content moved into `kb/`, so backlog items #4/#13/#14/#15/#25 point at dead paths and need re-filing. 12 new items queued (28–38 + a path-reconciliation meta-issue). Linear MCP still unavailable — 7th cycle in a row, now with a third distinct symptom (connector installed but `enabledInChat: false` for this session); recommend checking connector settings directly instead of retrying. |
