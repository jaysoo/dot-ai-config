...
|------|------|------|-------|
| 2026-07-24 | nrwl/nx | [nx-astro-docs-staleness-2026-07-24.md](./nx-astro-docs-staleness-2026-07-24.md) | Targeted 3-smell pass via 3 parallel scan agents over disjoint doc sections (480 files total). 7 new confirmed findings: stale "Supported versions" table in reference/releases.mdoc, self-contradicting Nx-23 removal note in reference/nx-json.mdoc, and 5 docs-vs-`packages/*/schema.json` option drifts (Next `serveStaticTargetName` undocumented deprecation, Storybook invalid `uiFramework` enum value + option-name typo, React Native missing `syncDepsTargetName`/`upgradeTargetName`, plus a recurring `buildDepsTargetName`/`watchDepsTargetName` omission across 4 plugin pages). Re-verified prior item #16 (bundling-node-projects.mdoc EOL `node18` target) independently — still open. Backlog now 33 Linear issues queued — MCP still unavailable (7th cycle in a row; this time `enabledInChat: false`, a third distinct failure symptom across cycles — looks like a persistent connector-enablement issue worth checking directly in claude.ai connector settings). |
