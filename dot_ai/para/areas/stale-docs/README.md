...
|------|------|------|-------|
| 2026-07-26 | nrwl/nx | [nx-astro-docs-staleness-2026-07-26.md](./nx-astro-docs-staleness-2026-07-26.md) | Full re-sweep of all 480 mdoc files, 13 parallel agents. Biggest finding: 8 KB pages teach the deprecated/unsupported Module Federation host/remote generators (`@nx/react:host`/`:remote`, `@nx/angular:host`/`:remote` — removed in Nx v24, Angular MF unsupported entirely) with zero deprecation notice, despite a correct v23+ replacement doc (consumer-and-provider.mdoc) already existing. Also new: reference/releases.mdoc version table one major behind, invalid `--unitTestRunner=vitest` flag breaking the Angular onboarding tutorial, createNodes/createNodesV2 guidance inverted in 3 places, Cypress migration guides pointing at an unsupported floor, 2 wrong plugin-option names (Nuxt, Storybook). Re-verified 9 prior findings still unfixed (nx-daemon.mdoc, publish-rust-crates.mdoc, terminal-ui.mdoc, Storybook testing-library ×4 files, etc). 23 Linear issues queued (9 carried + 14 new/expanded) — MCP still unavailable (7th cycle; this time `enabledInChat: false` — symptom keeps shifting cycle to cycle, worth fixing connector attachment for scheduled sessions specifically). |
