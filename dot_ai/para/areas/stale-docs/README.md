...
|------|------|------|-------|
| 2026-08-06 | nrwl/nx | [nx-astro-docs-staleness-2026-08-06.md](./nx-astro-docs-staleness-2026-08-06.md) | Targeted 4-smell pass, first cycle to also check `nrwl/ocean` (GitHub App permissions kb page, `nx-cloud` CLI commands vs. `client-bundle` source) — new ground, not covered by prior cycles. 14 new confirmed findings (8 stale Nx version anchors on newly-reorganized `kb/` pages, 4 CLI/schema option-drift issues incl. a wrong `nx.json` root-property table, 2 Ocean drift issues incl. `nx-cloud fix-ci`/`apply-locally`/`record` missing from the CLI reference entirely). Node/npm/framework sweep came back fully clean. Docs site was reorganized since 07-10 (files moved `guides/`→`kb/`, `extending-nx/`→`kb/`); re-verified 3 prior open items still present at their new paths. Backlog now 37 Linear issues queued — MCP still unavailable (7th cycle, 3rd distinct failure symptom: this time `enabledInChat: false` despite org-level connection, different from 07-10's "enabled but empty"). |
