...
|------|------|------|-------|
| 2026-07-31 | nrwl/nx | [nx-astro-docs-staleness-2026-07-31.md](./nx-astro-docs-staleness-2026-07-31.md) | Full sweep of all 478 mdoc files (7 parallel agents by folder), same 3-smell scope. 6 new confirmed findings (playwright preset options, releases.mdoc stale support table, nx-json.mdoc task-options root-vs-runner mixup, sandboxing.mdoc `--inputs/--outputs` flags that don't exist, angular tutorial invalid `--unitTestRunner=vitest`, create-server.mdoc documenting a nonexistent API). Also re-verified 3 known-open items still unfixed (#4 compose-executors "builder", #16 bundling-node-projects `node18`, #22 nx-daemon.mdoc useDaemonProcess placement). Backlog now 33 Linear issues queued — MCP still unavailable (**7th cycle in a row**; this time `enabledInChat: false` — connector authenticated at org level but not toggled on for this chat, a third distinct failure mode across 7 cycles). |
