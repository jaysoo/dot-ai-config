...
|------|------|------|-------|
| 2026-07-19 | nrwl/nx | [nx-astro-docs-staleness-2026-07-19.md](./nx-astro-docs-staleness-2026-07-19.md) | Targeted 3-smell pass, same scope as 07-10. 6 new confirmed findings: `nx.json` `maxCacheSize` doc claims a fabricated 10GB cap, `how-caching-works.mdoc` describes the wrong default output-fallback behavior, `next/introduction.mdoc` has 2 issues (Next.js floor overstated, `serveStaticTargetName` not marked deprecated), `adding-assets-react.mdoc` SVGR/Rspack removal has now actually shipped in Nx 23 (escalation of a prior "forward-looking" finding), plus 2 minor "Nx 20+" anchors folded into the existing cleanup bucket. Re-verified 9 known items still unfixed. Backlog now 33 Linear issues queued — MCP still unavailable (**7th cycle**, and the failure symptom changed for the 3rd time running: this cycle Linear shows `enabledInChat: false`, not even toggled on for the session — worth investigating directly rather than deferring again). |
