...
|------|------|------|-------|
| 2026-08-02 | nrwl/nx | [nx-astro-docs-staleness-2026-08-02.md](./nx-astro-docs-staleness-2026-08-02.md) | Targeted 3-smell pass, full 478-file sweep for versions (agents 1-2) + ~20-file spot-check for option drift (agent 3). Re-verified 6 prior findings still unfixed (incl. publish-rust-crates.mdoc contradiction, terminal-ui.mdoc Windows notice, react bundler conflation, bundling-node-projects.mdoc node18 target); 3 genuinely new option-mismatch findings (`--ci=azure-pipelines` invalid enum, `--unitTestRunner=vitest` invalid for `@nx/angular:library`, non-existent `nx show target --inputs/--outputs` flags) plus 1 new page-framing issue (updating-version-references.mdoc points to a stale "v20 version of the website"). Backlog now 33 Linear issues queued — MCP still unavailable (**7th cycle in a row**, 3rd distinct failure symptom: this time `enabledInChat: false` on the Linear connector — recommend a human check connector settings directly rather than retrying again next cycle). Docs site was reorganized since last cycle: several `guides/Nx Cloud/` and `reference/Nx Cloud/` files moved into `kb/`. |
