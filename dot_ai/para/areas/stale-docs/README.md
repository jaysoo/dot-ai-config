...
|------|------|------|-------|
| 2026-07-27 | nrwl/nx | [nx-astro-docs-staleness-2026-07-27.md](./nx-astro-docs-staleness-2026-07-27.md) | Full re-sweep of all 480 mdoc/mdx files via 6 parallel section agents, each cross-checking docs against `packages/**` source. 9 prior findings re-confirmed still open (Storybook addon cluster expanded from 3→10 files); 4 prior findings possibly fixed (not re-flagged — needs verification before closing); ~22 new findings (nx show target --inputs/--outputs don't exist, deprecated MF host/remote generators presented as current, releases.mdoc version table stale, several outdated generator/API examples). Running backlog now 40 open/new Linear issues. **Linear MCP status changed**: `ListConnectors` now shows `connected: true, enabledInChat: false` — connector is authenticated but toggled off for this chat, a one-click fix rather than the vaguer auth failures of prior cycles (7th consecutive cycle blocked). |
