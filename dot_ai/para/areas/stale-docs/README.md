...
|------|------|------|-------|
| 2026-07-07 | nrwl/nx | [nx-astro-docs-staleness-2026-07-07.md](./nx-astro-docs-staleness-2026-07-07.md) | Sixth scan; 15 parallel agents, all findings cross-checked against live `packages/*/src/**/schema.json` source per the verification rules below. **6 HIGH findings from 06-29 confirmed still unfixed** (compose-executors builder/tsConfig, migration-generators --project flag, publish-rust-crates useLegacyVersioning, terminal-ui Windows TUI notice, Storybook deprecated imports, bundling-node-projects node18 target). 14 new findings, incl. 7 Module Federation guides teaching generators deprecated in v23 with no notice, nx-json.mdoc misdocumenting 4 root-level options, releases.mdoc one major behind. 20 Linear issues queued — **MCP unavailable for the 6th consecutive scan**; root cause (broken Linear connection vs. nobody filing manually) needs a human decision. |
