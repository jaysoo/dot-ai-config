...
|------|------|------|-------|
| 2026-07-30 | nrwl/nx | [nx-astro-docs-staleness-2026-07-30.md](./nx-astro-docs-staleness-2026-07-30.md) | Targeted 3-smell pass via 4 parallel scan agents + 2 source-verification agents. 5 new confirmed findings (reference/releases.mdoc mislabels v22 as Current; ci-deployment.mdoc's stale "Vite support coming soon" note despite @nx/vite shipping for years; docker/login-action@v2 stale pin; obsolete cypress-v11-migration.mdoc vs. current Cypress 13-15 support; 2020-era browser-support.mdoc example output). Promoted prior needs-input NI-4 (as-provided-vs-derived.mdoc) to confirmed — source-verified the deprecated flags are actually gone, only the tense is stale. Re-verified 3 older findings still open (v1-nx-plugin-api.mdoc, bundling-node-projects.mdoc Node18, launch-template-examples.mdoc Node21). Cleared 5 false alarms via source verification (module-federation consumer/provider generators, jest --supportTsx, --with-target flag, NX_MIGRATE_SKIP_REGISTRY_FETCH wording, migrate agentic options, continuous target option — all accurate as documented). Backlog now 32 Linear issues queued — MCP still unavailable (7th cycle in a row; this time `enabledInChat: false` explicitly — connector authenticated but not enabled for this scheduled/unattended session, a new symptom shape). |
