...
|------|------|------|-------|
| 2026-07-28 | nrwl/nx | [nx-astro-docs-staleness-2026-07-28.md](./nx-astro-docs-staleness-2026-07-28.md) | Targeted 3-smell pass (476 mdoc files). Re-verified 4 prior findings still unfixed (v1-nx-plugin-api.mdoc, bundling-node-projects.mdoc Node 18 target, access-tokens.mdoc "authentication is changing", react/introduction.mdoc `--bundler` enums). 6 new findings, notably nx.json documenting 4 task-runner-only properties as valid at the config root, and a nonexistent `NX_RUNNER` env var. Backlog now 33 Linear issues queued — MCP still unavailable (7th cycle, 3rd distinct failure mode: this time `enabledInChat: false` for the session even though the connector is authenticated org-wide — see escalation note in this cycle's report). |
