...
|------|------|------|-------|
| 2026-08-03 | nrwl/nx | [nx-astro-docs-staleness-2026-08-03.md](./nx-astro-docs-staleness-2026-08-03.md) | Targeted 3-smell pass, not a full 478-file sweep. Live-verified current Nx (23.1.1), Node (26.5.1 latest / 20 now EOL), React (19.2.8) via `npm view` per this README's rules. Re-verified 3 prior findings still unfixed with sharper source evidence (v1-nx-plugin-api.mdoc has the *wrong removal version*, not just wrong tense — should say Nx 21 not 20; terminal-ui.mdoc Windows note confirmed stale by reading is-tui-enabled.ts directly; bundling-node-projects.mdoc still targets EOL node18). 3 new findings, all in nx-json.mdoc / environment-variables.mdoc (task-options table wrongly claims 4 deprecated-runner-only properties are root properties; 9 current root nx.json properties undocumented; NX_MULTI_MAJOR_MODE missing from env-var reference). Backlog now 30 Linear issues queued — MCP still unavailable (7th cycle in a row; symptom changed again — this time `enabledInChat: false` rather than the "connected but no tools" symptom from 07-10 — looks like a per-session toggle problem, worth checking session connector settings directly). |
