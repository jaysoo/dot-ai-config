...
|------|------|------|-------|
| 2026-08-08 | nrwl/nx + nrwl/ocean | [nx-astro-docs-staleness-2026-08-08.md](./nx-astro-docs-staleness-2026-08-08.md) | Targeted 4-smell pass, first cycle to also cross-check `nrwl/ocean` (GitHub App permissions, `nx-cloud` CLI commands) alongside `nrwl/nx`. 6 new confirmed findings: dead `NX_REJECT_UNKNOWN_LOCAL_CACHE=0` workaround, 6 live `nx-cloud` CLI commands missing from the "complete reference" page (incl. `fix-ci`/`apply-locally`), 4 drift points in `webpack-plugins.mdoc` (wrong default, removed-but-still-documented option, 5 undocumented options), and a cross-repo contradiction where the docs say the `Administration: Read & Write` GitHub App permission was removed but Ocean's own CLI remediation copy still tells users to grant it. Backlog now ~33 Linear issues queued — MCP still unavailable (7th cycle in a row; connector shows `connected` at org level but `enabledInChat: false` this session — needs a human to enable it for automated sessions, not another retry). |
