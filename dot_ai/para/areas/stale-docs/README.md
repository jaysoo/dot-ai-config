...
|------|------|------|-------|
| 2026-07-08 | nrwl/nx | [nx-astro-docs-staleness-2026-07-08.md](./nx-astro-docs-staleness-2026-07-08.md) | Nx 23 is now GA (npm dist-tags: latest 23.0.1) — this resolved several 06-29 "false positives" that had flagged v23 content as premature. New findings: releases.mdoc Supported Versions table not updated for v23 GA, nx-json.mdoc documents dead root-level task options (captureStderr/skipNxCache/encryptionKey only work in deprecated tasksRunnerOptions bag), releaseTag.preferDockerVersion type/default wrong, react/introduction.mdoc bundler list doesn't match app vs library generator schemas. Confirmed still-unfixed from 06-29: Deprecated/* future-tense Nx 20/21 notes, actions/checkout@v6 (doesn't exist), @v3/@v4 Actions pins now 2 majors behind (repo itself uses @v5). Node/React/Angular/TS version check: no new issues, docs current. Linear MCP unavailable for the 5th consecutive audit (since 06-17) — flagged as its own meta-issue to escalate. |
