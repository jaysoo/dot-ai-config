...
|------|------|------|-------|
| 2026-08-05 | nrwl/nx | [nx-astro-docs-staleness-2026-08-05.md](./nx-astro-docs-staleness-2026-08-05.md) | Targeted 3-smell pass. Corrected own agents' baseline mistake mid-run (initially read docs' self-labels as "Nx 22 current", live `npm view nx dist-tags` confirmed 23.1.1 — same error 06-29 made). Node 20 now fully past EOL as of this cycle's date. 3 new findings (yarn-pnp.mdoc stale `yarn@3.6.1` example, Storybook `uiFramework` `@storybook/vue-vite` not in schema enum, `--buildable` deprecated-for-React undocumented); 4 prior findings re-verified still open (v1-nx-plugin-api.mdoc tense, access-tokens.mdoc — now at kb/, moved from guides/Nx Cloud/ — react `--bundler` enum conflation, bundling-node-projects.mdoc node18 target). Backlog now 30 Linear issues queued — **7th consecutive cycle with Linear MCP unavailable**, symptom this time is `enabledInChat: false` (org-level `connected: true`); recommend investigating connector settings directly rather than continuing to retry per-audit. |
