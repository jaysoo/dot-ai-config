...
|------|------|------|-------|
| 2026-07-12 | nrwl/nx | [nx-astro-docs-staleness-2026-07-12.md](./nx-astro-docs-staleness-2026-07-12.md) | Targeted 3-smell pass, same scope as 07-10. Highest-value cycle yet: the feature-drift agent read `nx-json.ts`, `task-hasher.ts`, and `module-federation-deprecation.ts` directly and found 4 solid new drift issues — nx-json.mdoc documents 4 invalid root properties (High), 3 Module Federation guides describe generators that are deprecated and disappear in the very next major Nx v24 with zero notice (High), inputs.mdoc has the default tsconfig-hashing behavior backwards, how-caching-works.mdoc states the wrong default output fallback paths. Plus 1 new old-version finding (personal-access-tokens.mdoc, sibling of the known access-tokens.mdoc issue). Backlog now **32** Linear issues queued — MCP still unavailable (7th cycle in a row, same `installState: "unknown"` signature 4 cycles running with no self-recovery — recommended pausing the cadence until this is fixed directly in connector settings). |
