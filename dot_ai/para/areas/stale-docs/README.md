...
|------|------|------|-------|
| 2026-07-21 | nrwl/nx | [nx-astro-docs-staleness-2026-07-21.md](./nx-astro-docs-staleness-2026-07-21.md) | Targeted 3-smell pass. 4 new confirmed findings (best haul since 06-29): `reference/releases.mdoc` supported-versions table still shows v22 as Current on a 23.x repo; `nx-json.mdoc` Task-options table wrongly claims 4 properties live at the nx.json root when they actually nest under `tasksRunnerOptions.<name>.options`; `nx-json.mdoc` TUI section missing `suppressHints`; `unknown-local-cache.mdoc` documents a cache workaround that `legacy-cache.mdoc` itself says doesn't work anymore. Re-verified 2 known-open items (terminal-ui.mdoc Windows caveat, bundling-node-projects.mdoc Node 18 target, now on its 5th cycle open). Backlog now 31 Linear issues queued — MCP still unavailable (7th cycle in a row), but this cycle got a precise diagnosis: `ListConnectors` shows Linear `installState: connected` with **`enabledInChat: false`** — looks like a simple per-chat toggle, not a broken integration. |
