...
|------|------|------|-------|
| 2026-07-18 | nrwl/nx | [nx-astro-docs-staleness-2026-07-18.md](./nx-astro-docs-staleness-2026-07-18.md) | Full exhaustive re-sweep of all 509 mdoc files (13 parallel agents, per-directory source cross-checks). 25 new confirmed findings incl. Module Federation guides teaching 10 files' worth of deprecated generators, Angular Rsbuild package apparently discontinued, flagship plugin-authoring tutorial teaching deprecated createNodesV2, nx-json.mdoc documenting 4 nonexistent properties, releases.mdoc missing the current major entirely, and a stale version number on the "Verify installation" step of the highest-traffic getting-started page. 5 prior backlog items re-verified still broken (2 with expanded scope). Backlog now 51 Linear issues queued — MCP still unavailable (7th cycle in a row; symptom this time is `enabledInChat: false`, a third distinct failure signature across the run — recommend checking the connector directly rather than continuing to retry per-audit). |
