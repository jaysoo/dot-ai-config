...
|------|------|------|-------|
| 2026-07-03 | nrwl/nx | [nx-astro-docs-staleness-2026-07-03.md](./nx-astro-docs-staleness-2026-07-03.md) | Full re-scan of all 503 mdoc files, fanned out across ~25 parallel sub-agents by folder, each required to verify claims against `packages/` source before flagging. **Retracted 5 false positives from 2026-06-29** (H-1–H-4, M-1) — Nx 23.0.0 had actually shipped 2026-06-16, before that audit ran, so its "Nx 23 is unreleased" claims were wrong even at the time. Live baseline confirmed: Nx 23.0.1 current, Storybook 10 (not 8), Node 20 EOL confirmed past. 15 new High-severity findings incl. a stale `reference/releases.mdoc` support-version table, flagship tutorials teaching deprecated `CreateNodesV2`, several non-existent plugin options/generators (`previewStaticTargetName`, `upgrade-native`, `testTargetName`, `includeMobileBrowsers`), and 5 Storybook guides with a wrong addon-essentials claim. 28 Linear issues queued — MCP still unavailable (checked via ListConnectors/ToolSearch; connector shows installed but exposes no callable tools this session). |
| 2026-06-29 | nrwl/nx | [nx-astro-docs-staleness-2026-06-29.md](./nx-astro-docs-staleness-2026-06-29.md) | Full scan of all 503 mdoc files (new coverage: technologies/, extending-nx/, reference/, guides/, getting-started/). Key findings: broken Rust crates guide, Windows TUI stale, Storybook deprecated packages, compose-executors using "builder" not "executor". Note: scan used wrong Nx baseline (22 vs actual 23) and wrong GH Actions versions — several findings were false positives (retracted 2026-07-03). Agent verification rules added to this README. 26 Linear issues queued — MCP still unavailable. |
...
### 2026-07-03 addendum: "unreleased" claims need a release-date check, not just a version check

The 2026-06-29 scan flagged several pages for describing "Nx v23" features as already available, asserting v23 was unreleased. It was already released — `npm view nx version` at the time would have shown `23.0.1`, but whatever check that scan ran either used a stale cache or wasn't re-run per-finding. Before asserting a version is "not yet released" or "unreleased," always run `npm view nx time --json` (or `npm view nx versions --json` and eyeball the list) and check the actual publish date, not just a remembered "current" number from earlier in the scan — the baseline can be minutes stale by the time you're 400 files into a scan, and majors ship without much fanfare. When flagging forward-looking version language as premature, quote the specific version's publish date in the finding so it's trivially re-checkable.
