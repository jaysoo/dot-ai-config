...
|------|------|------|-------|
| 2026-07-11 | nrwl/nx | [nx-astro-docs-staleness-2026-07-11.md](./nx-astro-docs-staleness-2026-07-11.md) | First full re-sweep of all 511 mdoc files since 06-29, run as ~25 parallel section-owner agents cross-checking every claim against live `packages/` source. Biggest new find: a whole cluster of executors/generators/devkit types (Module Federation host/remote, `@nx/vitest:test`, `@nx/eslint:lint`, `@nx/cypress:cypress`, `createNodesV2`/`NxPluginV2`) that source has marked deprecated-removed-in-v24, still taught as current across ~25 files with zero warning. Also: 2 broken `nx show target --inputs/--outputs` CLI examples, a broken Nx Release ChangelogRenderer code sample, `reference/nx-json.mdoc` documenting nonexistent root-level properties, and Vite→Vitest split fallout in 3 tutorials. Backlog now 57 Linear issues queued — MCP still unavailable (7th cycle in a row, identical symptom to 07-10; recommend checking the connector directly rather than continuing to auto-retry). |
