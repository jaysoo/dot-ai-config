...
- `astro-docs/src/content/docs/getting-started/` - Getting started pages all have consistent "Next steps" bullet lists (PR #35024, 2026-03-26). DOC-579 (PR #36595, 2026-08-06) reshaped the two onboarding pages (`start-new-project.mdoc`, `start-with-existing-project.mdoc`) to share one layout: `llm_copy_prompt` at the top, a single prompt-block terminal with the command plus its sample run, then matching `## Next steps` and `## Keep learning` (tutorials, AI integrations, CI setup, video - editor setup was dropped from that list). Nx Cloud browser onboarding is deliberately absent from both; Cloud lives on `setup-ci.mdoc` and the CI features pages.
- `astro-docs/ec.config.mjs` - Expressive Code config + the `shell-prompts` plugin (DOC-579, PR #36595, 2026-08-06). A fence marked ```` ```shell {% meta="prompt=true" %} ```` may hold a command and its output together: `preprocessCode` strips the `$ ` prefixes before shiki runs (so the command highlights like any other shell block), `postprocessRenderedLine` re-adds the prompt as a `.shell-prompt` span and tags the remaining lines `.is-output`, and `postprocessRenderedBlock` rewrites the copy button so Copy hands over the commands alone. The two muting rules live at the bottom of `src/styles/global.css`. Five traps, in the order they bite:
  - EC options CANNOT live in `astro.config.mjs`. The `<Code>` component used by `src/pages/templates/[slug].astro` requires them JSON-serializable, and a plugin is not - the build fails telling you to create `ec.config.mjs`.
  - `ec.config.mjs` is imported by Node as plain ESM and CANNOT import a `.ts` module. astro-expressive-code catches the module-not-found, `continue`s, and returns `{}`. The config silently vanishes with no error. Keep that file self-contained.
  - hast normalizes `data-code` to the `dataCode` property. Matching on the dashed name finds nothing.
  - EC colors tokens via `span[style^='--']:not([class])`, so adding a class to a span drops its syntax color.
  - `addStyles()` from `postprocessRenderedLine` never reached the CSS bundle; the rules had to go in `global.css`. EC's own selectors use `:where()` (zero specificity), so a plain class rule wins.
  - Markdoc VALIDATES fence attributes - a bare `{% prompt=true %}` fails with "Invalid attribute". The starlight-markdoc preset exposes a passthrough `meta` attribute, so `{% meta="prompt=true" %}` gets arbitrary EC meta through without touching `markdoc.config.mjs`. Same trick works for any EC meta the preset does not model.
  - Keep it OPT-IN: `kb/root-level-scripts.mdoc` has a captured transcript whose `$ node ./generateDocsSite.js` line is yarn echoing, not something to run. Auto-detecting `$ ` would mangle it.
- `astro-docs/src/assets/tutorials/` - SVG diagrams and screenshots for tutorials
- **Previewing a built `astro-docs/dist` locally**: the site is built with `base: '/docs'`, so serving `dist` at the server root returns 200s for pages but 404s every stylesheet and script - the page renders as unstyled text and it looks like the build broke. Serve it one level up under a `docs` name instead: `mkdir -p /tmp/site && ln -sfn <repo>/astro-docs/dist /tmp/site/docs && npx serve /tmp/site -p 8099`, then open `http://127.0.0.1:8099/docs/<page>/`. Trailing slash matters (`trailingSlash: 'never'` applies to the deployed site, not the static file server).
...
More tells from DOC-573 (2026-08-06), now **codified in STYLE_GUIDE.md** by #36593 so they are
lookup-able instead of tribal:
- Rhetorical question as a setup device, then answering it. Grep for `?` outside FAQ headings.
- Significance clauses - `X is quiet, which is what makes it expensive`. Grep for "which is what makes", "and that's why".
- Short claim followed by a longer sentence re-explaining it. The old drama-beat rule only covered long-then-short; both orders now.
- Invented specifics that are really the reader's call (`branches that live hours or days`). Say "short-lived".
- Inflated nouns - a "fleet" of CI machines.
- Colons whose tail is a list should become real bullets, not a run-on.
Also: announcing a count before a list ("Two details matter here", "Two refinements", "Two habits")
appeared three times in one page. Not yet a STYLE_GUIDE rule, but the same fingerprint.
...
### 2026-08-06 - DOC-579: Getting-started cleanup, intro + Start a New Project + Add to Existing (MERGED #36595)
- **Branch**: `DOC-579` | **Merge**: `130727796c` (2026-08-06) | **PR**: https://github.com/nrwl/nx/pull/36595 | Polygraph `noble-osprey-dd3ebfa3` (single repo nrwl/nx) | Plan: `dot_ai/2026-08-06/tasks/doc-579-getting-started-cleanup.md`
- **What**: de-emphasized Nx Cloud onboarding across the three getting-started entry points. `getting-started/start-new-project.mdoc` is create-nx-workspace only (the "Option 2: Create via Nx Cloud" section and `src/assets/getting-started/nx-cloud-starting-screen.avif` deleted); `getting-started/start-with-existing-project.mdoc` keeps `nx init`; `getting-started/intro.mdoc` lost both `callout type="deepdive"` blocks. First-time users pick CNW or `nx init`, and Cloud stays on the pages where it is the subject (CI setup).
- **Shared page shape for both onboarding pages**: `llm_copy_prompt` agent prompt at the top, one terminal block holding the command plus its sample run, then matching `## Next steps` (commands, then caching + plugins) and `## Keep learning` (tutorials, AI integrations, CI setup, video). Editor setup was swapped out for AI integrations + CI setup. This supersedes the PR #35024 (2026-03-26) "Next steps links to tutorial series, editor setup, and CI" convention noted under `astro-docs/` in Directory Overview.
- **Templates**: the page no longer enumerates the four CNW prompt starters (`empty`/`react`/`angular`/`typescript`) - users see them in the prompt. It points at `/docs/templates` and shows one `--template=nrwl/tanstack-start-template` example. The short aliases resolve in `packages/create-nx-workspace/src/create-workspace.ts`; the other 8 gallery templates use their `nrwl/<repo>` slug, and `src/pages/templates/[slug].astro` renders the exact command via `installCommand()` in `src/data/templates.ts`.
- **NEW `astro-docs/ec.config.mjs`** - Expressive Code plugin for prompt blocks, plus 2 rules in `src/styles/global.css`. See the "Expressive Code prompt blocks" entry under `astro-docs/` in Directory Overview for the mechanics and the five traps hit getting there.
- **Source material**: sample timings (`ready in 20s` / `ready in 12s`) and both agent prompts were lifted off the live cloud.nx.app/get-started page before it is retired. The AI prompt sits behind a "Set up with an AI agent" modal with separate tabs for init and CNW, so it is not in the initial DOM - needed a click via Playwright, not a fetch. The Cloud prompts end in `nx connect` + `nx-cloud onboard connect-workspace`; those steps were dropped for the docs versions.
- **Also**: fixed two pre-existing dangling `getting-started/intro#start-small-extend-as-you-grow` anchors in `kb/nx-and-angular.mdoc` (the heading is `#start-small-grow-as-needed`).
- **Process note**: Jack marked the PR ready mid-session while I was still amending, so a `--force-with-lease` landed on an open-for-review PR. Remote and local were identical so nothing was lost, but check `gh pr view --json isDraft` before amending, not just that the branch tip matches.

### 2026-08-05..06 - DOC-573: Monorepo CI best practices KB page (MERGED #36593)
- **Branch**: `DOC-573` | **Merge**: `7dd949e953` (2026-08-06) | **PR**: https://github.com/nrwl/nx/pull/36593 | Polygraph `vivid-moose-c9937bd5` (single repo nrwl/nx) | Split-out fix `e056ef6320` / #36588
- **What**: new `astro-docs/src/content/docs/kb/monorepo-ci-best-practices.mdoc` (~3000 words) built to outrank Buildkite's #1 result. Sections ordered by payoff, not by feature. Also added a `## Plugins config` section to `astro-docs/src/content/docs/reference/nx-json.mdoc` documenting `projectsAffectedByDependencyUpdates`, which previously lived only on the affected feature page.
- **Page shape that survived review**: CI waste -> selective runs (`### Path-based filtering`, `### Graph-based affected detection`, `### Choose the right base commit`) -> task caching (`### Keep cache keys correct`) -> cores per machine -> distribution -> task splitting -> flaky tasks -> trunk-based development -> main-branch health -> ownership and boundaries -> single Nx CTA. Path filters and affected each get a full GitHub Actions workflow, labelled "without Nx" / "with Nx", so the contrast lands without leaving the page.
- **KB page mechanics confirmed**: a new `kb/*.mdoc` needs no `sidebar.mts` entry and no redirect (redirects only matter on move/rename/delete). `topics:` must match a label in `astro-docs/src/data/knowledge-base-topics.json` or `getTopicId` throws.
- **Code-block line highlighting**: ` ```yaml {% meta="{10-12}" %} `. The leading `// file` or `# file` comment **counts as line 1** even though it renders as a tab label rather than body. Verified against `technologies/test-tools/cypress/introduction.mdoc` (`meta="{14}"` only lands on `ciWebServerCommand` if the comment is counted) - do not guess this.
- **Accuracy corrections caught in review** (all were plausible-sounding and wrong): the comparison table said lockfile changes map to "the projects whose dependencies changed", but Nx defaults `projectsAffectedByDependencyUpdates` to `all`; cached results were described as anchored to the branch, when the cache is keyed by task inputs and only affected calculations use the branch baseline; file-level task splitting needs `nx connect` **plus** the test-runner plugin, so "automatically" was wrong; "CI runners are ephemeral" ignores self-hosted persistent runners; "the graph is derived from imports" ignores manifests and config.
- **`llm_copy_prompt` live-testing** (worth repeating for any agent prompt): built a throwaway 10-package pnpm monorepo, ran the prompt verbatim in a subagent, graded the report against known ground truth. v1 asked for wall-clock time, queue time and serial-wait share - none derivable from a repository, and nothing told the agent to decline, so it invited invented numbers. Rewritten so every check resolves to `Present` / `Absent` / `Needs CI access`, keyed to the page's own sections, with run-history metrics moved to an explicit hand-off list. **The first test was invalid**: harness instructions told the agent to decline unanswerable steps, which is exactly the behavior under test. Second run was clean and still declined.
- **Gotchas hit**: a markdown rewrap script deleted all 102 blank lines (empty string fell through the skip check), then a second version joined the YAML frontmatter into prose - vale caught that one with `E201 ... yaml: did not find expected key`. Any markdown transform script must skip fenced code, frontmatter, tables, lists **and** blank lines, and be verified by diffing blank-line count, word count, heading list, and link list before/after.
- **Messaging change**: Nx CLI and Nx Cloud may now be named as separate things, and features requiring Nx Cloud should say so. Supersedes the DOC-549 "one platform Nx" rule.
- **Open follow-up**: the page has no contextual inbound links. `setup-ci`, `ci-caching`, the affected feature page, and the CI features landing page are the candidates.
...
- **What**: 23.1.0 made github.com egress a hard CNW requirement (#35045 non-interactive default template since 22.7.0 + #36134 git clone -> fetch tarball). Sandboxed AI agents (npm-only egress) hit NETWORK_ERROR - 0 -> 87/day at GA, ~91% of the create-error spike, 88% AI. Root cause of the delta vs the git-clone era: git honors HTTPS_PROXY, Node fetch does not, so proxy-allowlist sandboxes broke at the client swap.
- **Fix shape** (v1 auto-fallback template->preset REJECTED by Jack - presets and templates generate different things): `invalidPresetToTemplateMap` in `bin/create-nx-workspace.ts` coerced `--preset empty` INTO the template download; replaced with exported `applyEmptyPresetAlias` (empty -> `Preset.TS`, npm-only `nx new`, deletes `argv.template` so appending the flag to a failed command escapes). Ordering-safe: alias mutates only argv, AI `legacyPresetToTemplateMap` reads rawArgs (map carries a "never add empty here" comment). Deleting the alias would route `empty` to third-party npm install of the unrelated `empty@0.10.1` (auto-confirmed for AI/CI).
- **Error taxonomy** (`src/utils/template/download-template.ts` + `src/utils/ai/ai-output.ts`): 404 = TEMPLATE_CLONE_FAILED (missing repo/branch), everything else (thrown fetch, 403 sandbox proxies per 7-day data, 407/429/5xx) = NETWORK_ERROR; `--preset=empty` guidance in message, AI hints, and `buildTemplateRequiredResult` pre-flight. Strict `^nrwl\/[\w.-]+$` slug check closed pre-existing `nrwl/../evil` cross-org tarball traversal (remote install-script execution).
- **Evidence**: create-* survey - sandbox-safe CLIs ship templates via npm (vite/next/t3/sv/vue bundle in package; expo/CRA publish template packages), github-at-runtime CLIs (turbo/astro/remix, next --example) all hard-fail. Long-term option noted: bundle default template in the package (create-vite/next pattern).
- **Gotchas hit**: CNW specs are NOT typechecked by the test target (`addTypecheckTarget: false`, tsconfig.lib excludes specs, jest via swc) - `Preset | 'empty'` narrowing broke `tsc -b tsconfig.spec.json` while the suite stayed green; smoke recipe for the built CLI = `pnpm pack` (npm chokes on `catalog:`), pin installed version to a published nx so `createSandbox` installs real packages, `NODE_OPTIONS=--require fetch-shim.cjs` to block github, `NX_CLOUD_API=http://127.0.0.1:9` to divert recordStat.
...
- **What**: `nx migrate --include=optional` crashed with `Cannot read properties of undefined (reading 'version')`. Surfaced migrating the ocean repo to latest Nx. Deterministic for ANY workspace, not ocean-specific.
- **Root cause** (`packages/nx/src/command-line/migrate/migrate.ts`, `generateMigrationsJsonAndUpdatePackageJson`): the 4th arg to `writePromptMigrationFiles` read `packageUpdates[walkedTargetPackage].version` UNGUARDED. Under `--include=optional`, `Migrator.applyIncludeFilter()` (`migrate.ts:711-724`) deletes every `requiredPackages` member from `packageUpdates`, and `resolveRequiredPackages()` (`migrate.ts:963-972`) always seeds that set with the target package itself -> that entry is deterministically `undefined`.
- **Fix**: hoisted the already-safe `packageUpdates[walkedTargetPackage]?.version ?? opts.targetVersion` (which lived ~18 lines below, used only by completion analytics) above the call and reused it. Net +8/-4. The version only names the AI-migrations output dir; `opts.targetVersion` is the correct fallback under optional.
- **Durable facts**:
  - The `--include` unit tests (`migrate.spec.ts` ~1098-1300) drive the **`Migrator` class directly** and assert on its returned `packageUpdates`; they structurally CANNOT reach the orchestration fn `generateMigrationsJsonAndUpdatePackageJson` where the crash lives. A Migrator-only test passes even with the bug present -> not a regression test for this class of crash.
  - To test the orchestration seam: `export`ed `generateMigrationsJsonAndUpdatePackageJson` (precedent in this file: `formatCommandFailure`, `resolveCreateCommits`, `resolveMigrationForRun` are exported solely for the spec), then drove it with a temp `root` + a stub `fetch` whose `packageGroup` puts a child in nx's required closure (so both `nx` and that child are dropped, leaving one optional pkg). It reads the real workspace `nx.json` via global `readNxJson()` (harmless: no `installation`); completion analytics no-op in tests (`customDimensions` undefined).
  - Adversarial scan of the full optional flow (`updatePackageJson`, `updateInstallationDetails`, `addSplitConfigurationMigrationIfAvailable`, `filterDowngradedUpdates`, run-migrations, analytics, next-steps messages) found NO sibling unguarded accesses - all others iterate present keys or already use `?.`.
- **Gotchas**: (1) **Run a single nx-package jest spec directly**: `NODE_OPTIONS=--experimental-vm-modules npx jest --config packages/nx/jest.config.cts --rootDir packages/nx <pattern> -t "<name>"`. The config is `jest.config.cts` (NOT `.ts`/`.cjs`) and `--rootDir packages/nx` is required, else "Can't find a root directory". (2) Polygraph `push_branch` is an MCP tool, not a shell `git push`, so it bypasses the `auth-proxy` wrapper entirely and never shows up in the Raycast request log.

### 2026-06-16..18 - CLOUD-4642: UTM tracking on clickable Cloud prompt links (DRAFT #36028)
- **Branch**: `CLOUD-4642` | **Worktree**: `/Users/jack/projects/nx-worktrees/CLOUD-4642` | **PR**: https://github.com/nrwl/nx/pull/36028 (DRAFT) | **Polygraph**: `cli-utm-99e98561` (nrwl/nx only) | Linear CLOUD-4642 (Orca/Nx Cloud)
- **What**: The `https://nx.dev/nx-cloud` marketing link in CLI cloud-setup prompt footers (create-nx-workspace, nx init, nx migrate, nx connect) is now a clickable OSC 8 hyperlink. Visible text stays the clean URL; click target carries `utm_source=nx-cli` + a per-command `utm_medium`. Terminals without OSC 8 fall back to the plain URL.
- **Files**:
  - `packages/nx/src/utils/terminal-link.ts` (NEW) + `packages/create-nx-workspace/src/utils/terminal-link.ts` (NEW, byte-identical copy): generic `terminalLink(text, url)` (OSC 8 framing), `supportsHyperlinks()` (vendored from `supports-hyperlinks` - FORCE_HYPERLINK / TTY / TERM_PROGRAM / VTE matrix), exported `parseVersion()`. Cross-ref "keep in sync" comment; duplication mirrors output.ts / package-manager.ts (CNW can't depend on nx).
  - `packages/nx/src/utils/ab-testing.ts`: `NX_CLOUD_URL` const + `nxCloudHyperlink(utmMedium)` helper. Footers DROP the bare URL; `connect-to-nx-cloud.ts` `nxCloudPrompt(key, utmMedium)` appends `nxCloudHyperlink(medium)` at render. `utmMediumForCommand(command)`: migrate->nx-migrate, view-logs->nx-connect, else->nx-init.
  - `packages/create-nx-workspace/src/utils/nx/ab-testing.ts`: `NX_CLOUD_URL` + a baked `NX_CLOUD_HYPERLINK = terminalLink(NX_CLOUD_URL, '...utm_medium=create-nx-workspace')` const embedded directly in the 3 footers; `prompts.ts` renders the footer as-is.
- **Design (durable)**: medium is per-COMMAND, not per-message. nx core's `setupNxCloud` footer is SHARED by `nx init` and `nx migrate` (different mediums) -> can't bake one const, so the helper is applied at render. CNW is single-medium -> baked const inlined in footers. Reworked from an initial `linkifyNxCloudUrl(footer, medium)` string-`.replace()` layer (review: silent no-op if a footer drifts) to const/helper - the bare URL no longer lives in any footer, so tracking drift is impossible by construction.
- **Gotchas**: (1) **Write/Edit tools STRIP raw terminal control bytes** (ESC `0x1B`, BEL `0x07`) - a literal OSC/BEL pasted into source becomes `]` / empty. Use `\u001B]` / `\u0007` ESCAPE-TEXT literals in source and verify with `cat -v` / `od -c`. Converting via inline `node -e` under fish mangles the backslashes (`(eval): unmatched "`) -> write a `.cjs` script file and run it. The same literal-vs-escape-text mismatch broke several `Edit` old_string matches. (2) the baked `NX_CLOUD_HYPERLINK` const is IMPORT-TIME evaluated -> in jest (non-TTY) it's the bare URL; test the UTM via `jest.resetModules()` + `requireActual` with `FORCE_HYPERLINK=1`, or test the call-time helper directly. (3) self-healing CI rerun commits drift the branch HEAD (see the NXC-4548 entry's gotcha).
