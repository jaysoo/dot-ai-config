...
### Dependency Vulnerability Remediation via pnpm Overrides (2026-07-30)
**Last Updated:** 2026-07-30
**Linear:** CLOUD-4927 (HIGH container), CLOUD-4936 (MEDIUM container), CLOUD-5066 (RR7-blocked)
**PRs:** #12575, #12614, #12629, #12630, #12631, #12656

How trivy findings against `nxdev/nx-cloud-frontend` and `nxdev/polygraph-frontend` get fixed.
Companion to the `ocean-trivy-verify` skill, which covers building and scanning the image; this
section covers the npm-advisory and override side.

#### Two scan surfaces, two different fixes

A finding lands in one of two places, and confusing them wastes hours:

1. **App `node_modules`** - the Docker build runs `pnpm install --prod` against
   `dist/apps/<app>/package.json` + a pruned lockfile, so this is a subset of our lockfile.
   Fixed with `pnpm.overrides`.
2. **The globally installed npm** - both Dockerfiles do `RUN npm install -g npm@<ver>`, and npm
   vendors its own `sigstore`, `tar`, `minimatch`, `brace-expansion`, `undici` and more under
   `node_modules/`. Only fixable by bumping the pinned npm version. To check what a given npm
   ships: `npm pack npm@<ver>`, untar, read `package/node_modules/<pkg>/package.json`.

If a flagged package is not in `pnpm-lock.yaml` at all (e.g. `sigstore`), it is surface 2.

#### Files Involved
- `package.json` -> `pnpm.overrides` - the real overrides, what pnpm actually resolves
- `apps/nx-cloud/package.json` -> `pnpm.overrides` - mirror; `nx prune` only preserves *this* file's
  overrides into the Docker manifest (CLOUD-4127), so a root-only override silently ships vulnerable
- `apps/polygraph/package.json` -> `pnpm.overrides` - same hazard, same requirement
- `tools/scripts/validate-pnpm-overrides.mjs` - CI gate (`.github/workflows/ci.yml`), exits 1 on any
  key/value mismatch between root and `apps/nx-cloud`
- `apps/nx-cloud/Dockerfile`, `apps/polygraph/Dockerfile` - the `npm install -g npm@<ver>` pin
- `libs/nx-packages/*/THIRD_PARTY_LICENSES.txt` - regenerated by the conformance
  `powerpack-third-party-licenses` rule; goes stale on any version change

#### Design Decisions & Gotchas
- **Verify every advisory range against the GitHub advisory API, never the Linear ticket.** The
  tickets are generated from trivy output and have been wrong more than once.
  `curl -s "https://api.github.com/advisories?cve_id=CVE-YYYY-NNNNN" | jq '.[0].vulnerabilities'`
  gives `vulnerable_version_range` and `first_patched_version` per package. CLOUD-4930 claimed
  undici was fixed in 6.26.0; the advisory and the v6.27.0 release notes say 6.27.0. CLOUD-4936 hit
  the same thing with uuid.
- **`first_patched_version: null` means no fix exists for that line at all**, not "unknown". That is
  what makes CVE-2026-53668 unfixable on react-router-dom 6.x.
- **`validate-pnpm-overrides.mjs` only compares root against `apps/nx-cloud`.** `apps/polygraph`
  drifts silently; its tar override sat at 7.5.20 while root was 7.5.22. Mirror all three by hand.
  Worth extending the validator.
- Scope override keys per major or per range, never blanket: `js-yaml@3`/`js-yaml@4`,
  `brace-expansion@1/2/5`, `picomatch@<2.3.2`, `path-to-regexp@<0.1.13`. A blanket key pins any
  future consumer that legitimately wants a newer major.
- **No override without a transitive requirer.** If a package is only a direct dep, bump the
  manifest instead. Parse requirers out of the lockfile's `snapshots:` section rather than reading
  `node_modules/.pnpm`, which carries stale copies from earlier installs.
- Advisories often name only the newest line even when older lines were patched too. brace-expansion
  CVE-2026-14257 lists `<= 5.0.7` only, but 1.1.18 and 2.1.4 carry the same `MAX_LENGTH` backport.
  Confirm by unpacking both tarballs and diffing for the guard before assuming a line is stuck.
- A newer npm is not always a fix: npm 11.18.0, 11.19.0 and 12.0.2 all vendor brace-expansion 5.0.7.
  When no npm release carries the patch, the options are wait or a scoped rego ignorePolicy in
  cloud-infrastructure.
- Lockfile regeneration after an override change is fine (`pnpm install --lockfile-only`) and stays
  scoped to the touched packages. The surgical hand-edit trick from CLOUD-4926 is only for a
  single-package bump where a full resolve would rewrite unrelated peer entries.
- Validate with `CI=true pnpm install --frozen-lockfile`, which is the real CI check.
- `pnpm nx-cloud conformance` needs a valid Nx Cloud PAT and fails in the agent sandbox with
  "Invalid Credentials". For a version-only change, hand-edit the `Package: <name> (<ver>)` line
  after confirming the new version's LICENSE is byte-identical, and let CI confirm the gate.
- Ocean vulnerability PRs do **not** get a version plan. Precedent: #12575, #12614, #12629.

## Personal Work History

### 2026-07-30

- **CLOUD-4927: Frontend/Polygraph HIGH vulnerabilities** (branch `CLOUD-4927`, commit `ce67755bdc`, draft PR #12656; Polygraph session `sharp-puma-7f09fb0e`)
  - Triaged all 20 CVE sub-issues against the GitHub advisory API. 8 fixed here, 2 closed Duplicate of CLOUD-4985 (Nicole's OTel #12631), 9 Canceled as already patched by #12575/#12614/CLOUD-4926, 4 RR7-blocked moved to a new CLOUD-5066 in the Remix V2 Migration project.
  - Overrides mirrored across root + both apps: brace-expansion 1.1.13/2.0.3/5.0.6 -> 1.1.18/2.1.4/5.0.9, js-yaml@4 4.2.0 -> 4.3.0, undici ^6.23.0 -> ^6.27.0, new `lodash-es` 4.18.0 / `path-to-regexp@<0.1.13` 0.1.13 / `@grpc/grpc-js` ^1.14.4, plus the `apps/polygraph` tar 7.5.20 -> 7.5.22 drift.
  - Caught a live miss: the ticket's "undici fixed in 6.26.0" is wrong (advisory + release notes say 6.27.0). We were on 6.26.0 and would have shipped still-vulnerable. See the advisory-API rule in the section above.
  - Rebased mid-session onto `532113a400` after #12631 merged; lockfile was the only conflict. Post-rebase finding: main still resolves `@grpc/grpc-js` 1.14.3 even after the OTel bump to 2.10.0/0.221.0, so the `^1.14.4` override is load-bearing rather than redundant.
  - Residual that will keep reporting: npm vendors brace-expansion 5.0.7 (CLOUD-4932). Not Remix-related, needs a later npm pin or an ignorePolicy.
  - Notes: `.ai/2026-07-30/tasks/cloud-4927-frontend-polygraph-high-vulns.md`
...
- **Public sandbox status badge** (branch `badge-sandbox-4c2e7734`, commit `5f87e43fb4`, PR #11878 merged; Polygraph session `badge-sandbox-4c2e7734`, Linear CLOUD-4623 linked)
  ...
  - Polygraph `push_branch` cannot push amended commits (pull-rebase conflict; also a user-rejected push_branch call may already have pushed the pre-amend commit). Recovery: `git fetch origin <branch>` then `git push --force-with-lease` directly, then `create_pr` works normally.
...
### JAVA_HOME for any `nx` command (2026-07-30)

`@nx/gradle` builds the project graph by running `./gradlew`, so **every** `nx` command fails
without a JDK 21, even for pure TS/React targets. `AGENTS.md` suggests
`/Library/Java/JavaVirtualMachines/jdk-21.jdk/Contents/Home`, which does **not** exist on Jack's
machine (that dir only has adoptopenjdk-14, jdk-12, and two jdk-8s). The working path comes from
mise:

```bash
JAVA_HOME=/Users/jack/.local/share/mise/installs/java/temurin-21.0.10+7.0.LTS \
MISE_TRUSTED_CONFIG_PATHS="$PWD" pnpm nx <target> <project>
```

Confirm with `mise where java` if the version bumps. Shell state does not persist between agent Bash
calls, so prepend it inline every time. A wrong `JAVA_HOME` fails as
`Failed to process project graph` and only names the real cause under `--verbose`.
