...
for repo in \
  ...
  anthropics/gemini-code modelcontextprotocol/specification \
...
9. **scan-ai-landscape** — AI dev tools landscape scan
   - Subagent instruction: "Run the AI landscape scan. In addition to the
     ...
     - Models advertised or benchmarked as strong at **vulnerability
       ...
       'Gemini Mythos' release that finds CVEs easily). These shift the
...
- The full command file content (read from `dot_gemini/commands/<name>.md`)
- Any repo paths
- Current date/month
