...
Also reflect on our current converstation/context and provide updates to $HOME/jack/projects/dot-ai-config/dot_gemini/GEMINI.md to prevent any mistakes or confusion that YOU had while completing the current task.
...
CRITICAL: Before you update GEMINI.md show me your suggested updates and I will approval/decline.
...
It also helps improve Gemini CLI itself by ensuring that mistakes done are not repeated:
- GEMINI.md is updated in $HOME/jack/projects/dot-ai-config/dot_gemini/GEMINI.md
...
- Look for mistakes or places where I corrected you and make sure you do not repeat the same mistakes by updating GEMINI.md
  - If a correction can be distilled into a reusable skill (e.g. a workflow, a multi-step process), suggest creating a new skill in `$HOME/projects/dot-ai-config/dot_gemini/skills/` instead of bloating GEMINI.md
  - Only add to GEMINI.md if it's a critical, cross-cutting rule that must always be top-of-mind
- **Identify reusable skill patterns**: Look for multi-step workflows that were repeated 2+ times during the session or that the user asked for explicitly. Signs of a skill candidate:
  ...
  - Suggest skills in `$HOME/projects/dot-ai-config/dot_gemini/skills/` with clear trigger words and a structured process
- Review commands that needed user permission during this session and suggest safe, non-destructive additions to `$HOME/projects/dot-ai-config/dot_gemini/settings.json` allowlist
